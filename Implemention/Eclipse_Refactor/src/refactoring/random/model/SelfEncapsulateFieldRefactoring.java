...
//			return result;
		fRoot = new RefactoringASTParser(IASTSharedValues.SHARED_AST_LEVEL).parse(fField.getCompilationUnit(), true,
				pm);
	...
	private boolean processCompilerError(RefactoringStatus result, ASTNode node) {
		...
		result.addFatalError(
				Messages.format(RefactoringCoreMessages.SelfEncapsulateField_compiler_errors_field, new String[] {
						BasicElementLabels.getJavaElementName(fField.getElementName()), messages[0].getMessage() }));
		return true;
	}

	private String getMappingErrorMessage() {
		return Messages.format(RefactoringCoreMessages.SelfEncapsulateField_cannot_analyze_selected_field,
	...
	private static void checkName(RefactoringStatus status, String name, List<IMethodBinding> usedNames, IType type,
			boolean reUseExistingField, IField field) {
			...
			if (selector.equals(name)) {
				if (!reUseExistingField) {
					status.addFatalError(Messages.format(RefactoringCoreMessages.SelfEncapsulateField_method_exists,
							new String[] {
									BindingLabelProviderCore.getBindingLabel(method,
					...
					if (methodIsStatic && !isStatic)
						status.addWarning(Messages.format(
								RefactoringCoreMessages.SelfEncapsulateFieldRefactoring_static_method_but_nonstatic_field,
								new String[] { BasicElementLabels.getJavaElementName(method.getName()),
										BasicElementLabels.getJavaElementName(field.getElementName()) }));
					if (!methodIsStatic && isStatic)
						status.addFatalError(Messages.format(
								RefactoringCoreMessages.SelfEncapsulateFieldRefactoring_nonstatic_method_but_static_field,
								new String[] { BasicElementLabels.getJavaElementName(method.getName()),
										BasicElementLabels.getJavaElementName(field.getElementName()) }));
					return;
				}
		...
		if (reUseExistingField)
			status.addFatalError(Messages.format(
					...
					new String[] { BasicElementLabels.getJavaElementName(name),
							BasicElementLabels.getJavaElementName(type.getElementName()) }));
	}
...
//			return result;
		...
		ICompilationUnit[] affectedCUs = RefactoringSearchEngine.findAffectedCompilationUnits(pattern,
				RefactoringScopeFactory.create(fField, fConsiderVisibility), subPm, result, true);
...
//			return result;
		...
		ITypeBinding declaringClass = ASTNodes.getParent(fFieldDeclaration, AbstractTypeDeclaration.class)
				.resolveBinding();
		...
		ownerDescriptions.addAll(addGetterSetterChanges(fRoot, fRewriter, owner.findRecommendedLineSeparator(),
				usingLocalSetter, usingLocalGetter));
		...
		public boolean visit(TypeDeclaration node) {
			...
			while (typeBinding != null) {
				...
				for (IMethodBinding methodBinding : methodBindings) {
					if (methodBinding.getName().equals(getGetterName())) {
//							&& methodBinding.getParameterNames().length == 0) {
						fTypeName = typeBinding.getName();
						fGetterConflict = true;
						throw new AbortSearchException();
					}
					if (methodBinding.getName().equals(getSetterName())) {
//							&& methodBinding.getParameterNames().length == 1) {
						if (methodBinding.getParameterTypes()[0]
								.isEqualTo(fFieldDeclaration.resolveBinding().getType())) {
							fTypeName = typeBinding.getName();
							throw new AbortSearchException();
						}
					}
	...
	private void createEdits(ICompilationUnit unit, ASTRewrite rewriter, List<TextEditGroup> groups,
	...
	public Change createChange(IProgressMonitor pm) throws CoreException {
		...
		final String header = Messages.format(
				...
				new String[] { JavaElementLabelsCore.getElementLabel(fField, JavaElementLabelsCore.ALL_FULLY_QUALIFIED),
						JavaElementLabelsCore.getElementLabel(declaring, JavaElementLabelsCore.ALL_FULLY_QUALIFIED) });
		...
		comment.addSetting(
				Messages.format(RefactoringCoreMessages.SelfEncapsulateField_visibility_pattern, visibility));
	...
	private void checkCompileErrors(RefactoringStatus result, CompilationUnit root, ICompilationUnit element) {
		for (IProblem problem : root.getProblems()) {
			if (!isIgnorableProblem(problem)) {
				result.addWarning(Messages.format(RefactoringCoreMessages.SelfEncapsulateField_compiler_errors_update,
	...
	public static void checkMethodInHierarchy(ITypeBinding type, String methodName, ITypeBinding returnType,
			ITypeBinding[] parameters, RefactoringStatus result, boolean reUseMethod) {
		...
		if (method != null) {
			...
			if (reUseMethod) {
				result.addError(Messages.format(
						RefactoringCoreMessages.SelfEncapsulateFieldRefactoring_nosuchmethod_status_fatalError,
						BasicElementLabels.getJavaElementName(methodName)), JavaStatusContext.create(method));
			}
		}
	}
	...
	private List<TextEditGroup> addGetterSetterChanges(CompilationUnit root, ASTRewrite rewriter, String lineDelimiter,
			boolean usingLocalSetter, boolean usingLocalGetter) throws CoreException {
	...
	private TextEditGroup makeDeclarationPrivate(ASTRewrite rewriter, FieldDeclaration decl) {
		...
		VariableDeclarationRewrite.rewriteModifiers(decl, vdfs, includedModifiers, excludedModifiers, rewriter,
				description);
		return description;
	}
	...
	private MethodDeclaration createSetterMethod(AST ast, ASTRewrite rewriter, String lineDelimiter)
		...
		List<Dimension> extraDimensions = DimensionRewrite.copyDimensions(fFieldDeclaration.extraDimensions(),
				rewriter);
		...
		String body = CodeGeneration.getSetterMethodBodyContent(fField.getCompilationUnit(),
				getTypeName(field.getParent()), fSetterName, fieldAccess, fArgName, lineDelimiter);
		if (body != null) {
		...
		if (fGenerateJavadoc) {
			String string = CodeGeneration.getSetterComment(fField.getCompilationUnit(), getTypeName(field.getParent()),
					fSetterName, fField.getElementName(), ASTNodes.asString(type), fArgName,
					StubUtility.getBaseName(fField), lineDelimiter);
			if (string != null) {
	...
	private MethodDeclaration createGetterMethod(AST ast, ASTRewrite rewriter, String lineDelimiter)
		...
		Type returnType = DimensionRewrite.copyTypeAndAddDimensions(type, fFieldDeclaration.extraDimensions(),
				rewriter);
		...
		String body = CodeGeneration.getGetterMethodBodyContent(fField.getCompilationUnit(),
				getTypeName(field.getParent()), fGetterName, fField.getElementName(), lineDelimiter);
		if (body != null) {
		...
		if (fGenerateJavadoc) {
			String string = CodeGeneration.getGetterComment(fField.getCompilationUnit(), getTypeName(field.getParent()),
					fGetterName, fField.getElementName(), ASTNodes.asString(type), StubUtility.getBaseName(fField),
					lineDelimiter);
			if (string != null) {
				Javadoc javadoc = (Javadoc) fRewriter.createStringPlaceholder(string, ASTNode.JAVADOC);
				result.setJavadoc(javadoc);
	...
	public RefactoringStatus initialize(JavaRefactoringArguments arguments) {
		...
		if (handle != null) {
			...
			if (element == null || !element.exists() || element.getElementType() != IJavaElement.FIELD)
				return JavaRefactoringDescriptorUtil.createInputFatalStatus(element, getName(),
						IJavaRefactorings.ENCAPSULATE_FIELD);
			else {
		...
		if (name != null && !name.isEmpty())
			...
			return RefactoringStatus.createFatalErrorStatus(Messages
					.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_GETTER));
		name = arguments.getAttribute(ATTRIBUTE_SETTER);
		if (name != null && !name.isEmpty())
			fSetterName = name;
		else
			return RefactoringStatus.createFatalErrorStatus(Messages
					.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_SETTER));
		...
		if (encapsulate != null) {
			...
			return RefactoringStatus.createFatalErrorStatus(Messages
					.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_DECLARING));
		...
		if (insertion != null && !"".equals(insertion)) {//$NON-NLS-1$
			...
			try {
				...
				return RefactoringStatus.createFatalErrorStatus(Messages.format(
						RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_INSERTION));
			}
			fInsertionIndex = index;
		}
