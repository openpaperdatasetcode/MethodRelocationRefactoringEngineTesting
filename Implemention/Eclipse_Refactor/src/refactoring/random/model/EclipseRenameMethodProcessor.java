...
import refactoring.model.RefactoringScopeFactory;
import refactoring.model.RefactoringSearchEngine;

public class EclipseRenameMethodProcessor extends JavaRenameProcessor implements IReferenceUpdating, IDelegateUpdating {
	 ...
	 */
	public EclipseRenameMethodProcessor(IMethod method) {
	 ...
	 */
	protected EclipseRenameMethodProcessor(IMethod method, TextChangeManager manager, GroupCategorySet categorySet) {
		initialize(method);
	...
	protected void setData(EclipseRenameMethodProcessor other) {
	...
	public final RefactoringStatus checkNewElementName(String newName) {
		...
		if (status.isOK() && !Checks.startsWithLowerCase(newName))
			status = RefactoringStatus
					.createWarningStatus(fIsComposite
							? Messages.format(RefactoringCoreMessages.Checks_method_names_lowercase2,
		...
		if (Checks.isAlreadyNamed(fMethod, newName))
			status.addFatalError(
					fIsComposite
							? Messages.format(RefactoringCoreMessages.RenameMethodRefactoring_same_name2,
									new String[] { BasicElementLabels.getJavaElementName(newName),
											getDeclaringTypeLabel() })
	...
	protected RefactoringStatus doCheckFinalConditions(IProgressMonitor pm, CheckConditionsContext context)
		...
		try {
			...
			if (!Checks.isAvailable(fMethod)) {
				result.addFatalError(RefactoringCoreMessages.RenameMethodProcessor_is_binary,
						JavaStatusContext.create(fMethod));
				return result;
			}
			result.merge(Checks.checkIfCuBroken(fMethod));
			...
			if (newNameMethods.length == 0) {
				...
				if (outerTypes.length > 0) {
					// There exists a reference to a clashing method, where the reference is in a
					// nested type.
					...
					// a hierarchy contains one of outerTypes (or an outer type of an outerType,
					// recursively).
					...
					if (hasOldRefsInInnerTypes) {
						...
						// Examples: RenamePrivateMethodTests#testFail2(),
						// RenamePrivateMethodTests#testFail5()
						mustAnalyzeShadowing = true;
					} else {
						mustAnalyzeShadowing = false;
					}
	...
	private IType[] searchForOuterTypesOfReferences(IMethod[] newNameMethods, IProgressMonitor pm)
		...
		SearchPattern pattern = RefactoringSearchEngine.createOrPattern(newNameMethods,
		...
		if (pattern != null && scope != null && requestor != null)
	...
	private IMethod[] searchForDeclarationsOfClashingMethods(IProgressMonitor pm) throws CoreException {
		...
		SearchRequestor requestor = new SearchRequestor() {
			...
			public void acceptSearchMatch(SearchMatch match) throws CoreException {
				...
				if (method instanceof IMethod) // check for bug 90138: [refactoring] [rename] Renaming method throws
												// internal exception
					results.add((IMethod) method);
//				else
//					JavaManipulationPlugin.logErrorMessage("Unexpected element in search match: " + match.toString()); //$NON-NLS-1$
			}
		};
		new SearchEngine().search(pattern, SearchUtils.getDefaultSearchParticipants(), scope, requestor, pm);
	...
	private SearchResultGroup[] getOccurrences(IProgressMonitor pm, RefactoringStatus status,
			ReferencesInBinaryContext binaryRefs) throws CoreException {
...
//		result.merge(Checks.checkCompileErrorsInAffectedFiles(fOccurrences));
	...
	private RefactoringStatus analyzeRenameChanges(IProgressMonitor pm) throws CoreException {
		...
		try {
			...
			newDeclarationWCs = RenameAnalyzeUtil.createNewWorkingCopies(declarationCUs, fChangeManager,
					fWorkingCopyOwner, new SubProgressMonitor(pm, 1));
			...
			for (Iterator<IMethod> iter = fMethodsToRename.iterator(); iter.hasNext(); i++) {
				...
				ICompilationUnit newCu = RenameAnalyzeUtil.findWorkingCopyForCu(newDeclarationWCs,
						method.getCompilationUnit());
				IType typeWc = (IType) JavaModelUtil.findInCompilationUnit(newCu, method.getDeclaringType());
				if (typeWc == null) {
					// should not happen
					i--;
					wcOldMethods = CollectionsUtil
							.toArray(Arrays.asList(wcOldMethods).subList(0, wcOldMethods.length - 1), IMethod.class);
					wcNewMethods = CollectionsUtil
							.toArray(Arrays.asList(wcNewMethods).subList(0, wcNewMethods.length - 1), IMethod.class);
					continue;
				}
...
//			SearchResultGroup[] newOccurrences= findNewOccurrences(newMethods, newDeclarationWCs, new SubProgressMonitor(pm, 3));
			SearchResultGroup[] newOccurrences = batchFindNewOccurrences(wcNewMethods, wcOldMethods, newDeclarationWCs,
					new SubProgressMonitor(pm, 3), result);

			result.merge(RenameAnalyzeUtil.analyzeRenameChanges2(fChangeManager, fOccurrences, newOccurrences,
					getNewElementName()));
			return result;
		} finally {
			pm.done();
	...
	// Lower memory footprint than batchFindNewOccurrences. Not used because it is
	// too slow.
...
//	}
	...
	private SearchResultGroup[] batchFindNewOccurrences(IMethod[] wcNewMethods, final IMethod[] wcOldMethods,
			ICompilationUnit[] newDeclarationWCs, IProgressMonitor pm, RefactoringStatus status) throws CoreException {
		pm.beginTask("", 2); //$NON-NLS-1$

		SearchPattern refsPattern = RefactoringSearchEngine.createOrPattern(wcNewMethods,
				IJavaSearchConstants.REFERENCES);
		SearchParticipant[] searchParticipants = SearchUtils.getDefaultSearchParticipants();
		...
		try {
			otherWCs = RenameAnalyzeUtil.createNewWorkingCopies(needWCs.toArray(new ICompilationUnit[needWCs.size()]),
	...
	public Change createChange(IProgressMonitor monitor) throws CoreException {
		try {
			...
			final String header = Messages.format(RefactoringCoreMessages.RenameMethodProcessor_descriptor_description,
					new String[] {
							JavaElementLabelsCore.getTextLabel(fMethod, JavaElementLabelsCore.ALL_FULLY_QUALIFIED),
			...
			return new DynamicValidationRefactoringChange(descriptor,
					RefactoringCoreMessages.RenameMethodProcessor_change_name, list.toArray(new Change[list.size()]));
		} finally {
			monitor.done();
		}
	}
				...
				if (declarationsInThisCu.size() > 0) {
					...
					if (fDelegateUpdating) {
						...
						for (SearchMatch element : declarationsInThisCu) {
							MethodDeclaration method = ASTNodeSearchUtil
									.getMethodDeclarationNode((IMethod) element.getElement(), rewrite.getRoot());
	...
	protected final ReplaceEdit createReplaceEdit(SearchMatch searchResult, ICompilationUnit cu) {
		if (searchResult.isImplicit()) { // handle Annotation Element references, see bug 94062
			...
			if (JavaCore.INSERT.equals(cu.getJavaProject()
	 ...
	 * {@link RenameVirtualMethodProcessor} and
	 * {@link RenameNonVirtualMethodProcessor}
	 *
			...
			if (element instanceof IMethod) {
				...
				if (declaring != null && declaring.exists()) {
					...
					if (methods != null && methods.length == 1 && methods[0] != null) {
						if (!methods[0].exists())
							return JavaRefactoringDescriptorUtil.createInputFatalStatus(methods[0], refactoring,
									IJavaRefactorings.RENAME_METHOD);
						assignMethod(methods[0]);
						initializeWorkingCopyOwner();
					} else
						return JavaRefactoringDescriptorUtil.createInputFatalStatus(null, refactoring,
								IJavaRefactorings.RENAME_METHOD);
				} else
					return JavaRefactoringDescriptorUtil.createInputFatalStatus(element, refactoring,
							IJavaRefactorings.RENAME_METHOD);
			} else
				return JavaRefactoringDescriptorUtil.createInputFatalStatus(element, refactoring,
						IJavaRefactorings.RENAME_METHOD);
	...
	@Override
	public String getDelegateUpdatingTitle(boolean arg0) {
