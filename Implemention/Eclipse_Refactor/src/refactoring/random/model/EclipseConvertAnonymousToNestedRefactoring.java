...
public class EclipseConvertAnonymousToNestedRefactoring extends Refactoring {
	 ...
	 */
	public EclipseConvertAnonymousToNestedRefactoring(ICompilationUnit unit, int selectionStart, int selectionLength) {
	...
	public EclipseConvertAnonymousToNestedRefactoring(AnonymousClassDeclaration declaration) {
	...
	public EclipseConvertAnonymousToNestedRefactoring(JavaRefactoringArguments arguments, RefactoringStatus status) {
		this(null, 0, 0);
	...
	public boolean isLocalInnerType() {
		return ASTNodes.getParent(ASTNodes.getParent(fAnonymousInnerClassNode, AbstractTypeDeclaration.class),
	...
	public void setVisibility(int visibility) {
		Assert.isTrue(visibility == Modifier.PRIVATE || visibility == Modifier.NONE || visibility == Modifier.PROTECTED
...
//					RefactoringCoreMessages.ConvertAnonymousToNestedRefactoring_extends_local_class);
	...
	private void initAST(IProgressMonitor pm) {
		...
		if (fAnonymousInnerClassNode == null) {
			fAnonymousInnerClassNode = getAnonymousInnerClass(
					NodeFinder.perform(fCompilationUnitNode, fSelectionStart, fSelectionLength));
		}
		if (fAnonymousInnerClassNode != null) {
			final AbstractTypeDeclaration declaration = ASTNodes.getParent(fAnonymousInnerClassNode,
...
//					.createFatalErrorStatus(RefactoringCoreMessages.ConvertAnonymousToNestedRefactoring_another_name);
	...
	private List<IVariableBinding> getAllEnclosingAnonymousTypesField() {
		...
		final AbstractTypeDeclaration declaration = ASTNodes.getParent(fAnonymousInnerClassNode,
				AbstractTypeDeclaration.class);
		AnonymousClassDeclaration anonymous = (AnonymousClassDeclaration) ASTNodes.getParent(fAnonymousInnerClassNode,
	...
	private boolean classNameHidesEnclosingType() {
		ITypeBinding type = ASTNodes.getParent(fAnonymousInnerClassNode, AbstractTypeDeclaration.class)
				.resolveBinding();
		while (type != null) {
		...
		try {
			...
			if (accessesAnonymousFields())
				status.merge(RefactoringStatus.createErrorStatus(
						RefactoringCoreMessages.ConvertAnonymousToNestedRefactoring_anonymous_field_access));
			return status;
		} finally {
			pm.done();
		}
	...
	/*
	 * @see
	 * org.eclipse.jdt.internal.corext.refactoring.base.IRefactoring#createChange(
	...
	private ITypeBinding[] getImpliedClassCreationTypeParameters() {
		...
		// In assignments and variable declaration statements, a type parameter might
		// exist in the
		// left side of the statement and implied in the class creation (e.g.
		// Set<String> x= new HashSet<>();)
		ASTNode nodeOfInterest = ASTNodes.getFirstAncestorOrNull(creation, Assignment.class, FieldDeclaration.class,
	...
	private ITypeBinding[] getTypeParameters() {
		...
		if (fDeclareStatic) {
			...
			if (declaration != null) {
				...
				for (final Iterator<TypeParameter> iterator = declaration.typeParameters().iterator(); iterator
						.hasNext();) {
	...
	private RefactoringChangeDescriptor createRefactoringDescriptor() {
		...
		final String header = Messages
				.format(RefactoringCoreMessages.ConvertAnonymousToNestedRefactoring_descriptor_description, labels);
		final JDTRefactoringDescriptorComment comment = new JDTRefactoringDescriptorComment(projectName, this, header);
		comment.addSetting(Messages.format(RefactoringCoreMessages.ConvertAnonymousToNestedRefactoring_original_pattern,
				BindingLabelProviderCore.getBindingLabel(binding, JavaElementLabelsCore.ALL_FULLY_QUALIFIED)));
		comment.addSetting(
				Messages.format(RefactoringCoreMessages.ConvertAnonymousToNestedRefactoring_class_name_pattern,
		...
		comment.addSetting(Messages
				.format(RefactoringCoreMessages.ConvertAnonymousToNestedRefactoring_visibility_pattern, visibility));
		...
		arguments.put(JavaRefactoringDescriptorUtil.ATTRIBUTE_SELECTION,
				Integer.toString(fSelectionStart) + ' ' + Integer.toString(fSelectionLength));
		...
		ConvertAnonymousDescriptor descriptor = RefactoringSignatureDescriptorFactory
				.createConvertAnonymousDescriptor(projectName, description, comment.asString(), arguments, flags);
	...
	private void addArgumentsForLocalsUsedInInnerClass(ClassInstanceCreation newClassCreation) {
		for (final IVariableBinding binding : getUsedLocalVariables()) {
			...
			if (binding.isEnumConstant())
				name = ast.newQualifiedName(ast.newSimpleName(binding.getDeclaringClass().getName()),
	...
	private void copyArguments(CompilationUnitRewrite rewrite, ClassInstanceCreation newClassCreation) {
		Iterator<Expression> iter = ((ClassInstanceCreation) fAnonymousInnerClassNode.getParent()).arguments()
				.iterator();
		if (!iter.hasNext())
			return;
	...
	private void addNestedClass(CompilationUnitRewrite rewrite, ITypeBinding[] typeParameters,
			ITypeBinding[] impliedSuperTypeParameters) throws CoreException {
		final AbstractTypeDeclaration declarations = ASTNodes.getParent(fAnonymousInnerClassNode,
				AbstractTypeDeclaration.class);
		int index = findIndexOfFistNestedClass(declarations.bodyDeclarations());
		if (index == -1)
			index = 0;
		rewrite.getASTRewrite().getListRewrite(declarations, declarations.getBodyDeclarationsProperty())
				.insertAt(createNewNestedClass(rewrite, typeParameters, impliedSuperTypeParameters), index, null);
	}
	...
	private AbstractTypeDeclaration createNewNestedClass(CompilationUnitRewrite rewrite, ITypeBinding[] typeParameters,
		...
		for (int i = 0; i < bindings.length; i++) {
			...
			String[] fieldNameProposals = StubUtility.getVariableNameSuggestions(NamingConventions.VK_INSTANCE_FIELD,
					project, name, 0, fieldNames, true);
			fieldNames.add(fieldNameProposals[0]);

			if (fLinkedProposalModelCore != null) {
				LinkedProposalPositionGroupCore positionGroup = fLinkedProposalModelCore
		...
		if (doAddComments()) {
			...
			String string = CodeGeneration.getTypeComment(rewrite.getCu(), fClassName, parameterNames,
		...
		if (fLinkedProposalModelCore != null) {
			...
			ModifierCorrectionSubProcessorCore.installLinkedVisibilityProposals(fLinkedProposalModelCore,
					rewrite.getASTRewrite(), newDeclaration.modifiers(), false);
		}

		return newDeclaration;
	}

	private void updateAndMoveBodyDeclarations(CompilationUnitRewrite rewriter, IVariableBinding[] bindings,
			String[] fieldNames, List<BodyDeclaration> newBodyDeclarations, MethodDeclaration newConstructorDecl) {
		final ASTRewrite astRewrite = rewriter.getASTRewrite();
		final AST ast = astRewrite.getAST();

		final boolean useThisAccess = useThisForFieldAccess();

		int fieldInsertIndex = newConstructorDecl != null ? newBodyDeclarations.lastIndexOf(newConstructorDecl)
				: newBodyDeclarations.size();

		for (Iterator<BodyDeclaration> iterator = fAnonymousInnerClassNode.bodyDeclarations().iterator(); iterator
				.hasNext();) {
		...
		if (newConstructorDecl != null) {
			// move initialization of existing fields to constructor if an outer is
			// referenced
			...
			for (VariableDeclarationFragment fragment : getFieldsToInitializeInConstructor()) {
				...
				Expression replacement = (Expression) astRewrite.get(fragment,
	...
	private void createFieldsForAccessedLocals(CompilationUnitRewrite rewrite, IVariableBinding[] varBindings,
			String[] fieldNames, List<BodyDeclaration> newBodyDeclarations) throws CoreException {
		...
		for (int i = 0; i < varBindings.length; i++) {
			...
			if (doAddComments()) {
				String string = CodeGeneration.getFieldComment(rewrite.getCu(), varType.getName(), fieldNames[i],
						StubUtility.getLineDelimiterUsed(fCu));
				if (string != null) {
	...
	private MethodDeclaration createNewConstructor(CompilationUnitRewrite rewrite, IVariableBinding[] bindings,
			String[] fieldNames) throws JavaModelException {
		...
		if (!instanceCreation.arguments().isEmpty()) {
			...
			if (constructorBinding != null) {
				...
				for (int i = 0; i < parameterNames.length; i++) {
					...
					SingleVariableDeclaration param = newParameterDeclaration(ast, importRewrite, paramName,
							parameterTypes[i]);
					...
					if (fLinkedProposalModelCore != null) {
						LinkedProposalPositionGroupCore positionGroup = fLinkedProposalModelCore
								.getPositionGroup(KEY_PARAM_NAME_CONST + String.valueOf(i), true);
		...
		for (int i = 0; i < bindings.length; i++) {
			...
			String[] paramNameProposals = StubUtility.getVariableNameSuggestions(NamingConventions.VK_PARAMETER,
					project, baseName, 0, newParameterNames, true);
			...
			newStatements.add(newFieldAssignment(ast, fieldNameNode, paramNameNode,
					useThisAccess || newParameterNames.contains(fieldName)));

			if (fLinkedProposalModelCore != null) {
				LinkedProposalPositionGroupCore positionGroup = fLinkedProposalModelCore
						.getPositionGroup(KEY_PARAM_NAME_EXT + String.valueOf(i), true);
				positionGroup.addPosition(astRewrite.track(param.getName()), false);
				...
				fLinkedProposalModelCore.getPositionGroup(KEY_FIELD_NAME_EXT + i, true)
		...
		if (doAddComments()) {
			try {
				...
				String string = CodeGeneration.getMethodComment(fCu, fClassName, fClassName, allParamNames,
						new String[0], null, new String[0], null, StubUtility.getLineDelimiterUsed(fCu));
				if (string != null) {
					Javadoc javadoc = (Javadoc) astRewrite.createStringPlaceholder(string, ASTNode.JAVADOC);
	...
	private Statement newFieldAssignment(AST ast, SimpleName fieldNameNode, Expression initializer,
			boolean useThisAccess) {
	...
	private boolean isToBeInitializerInConstructor(VariableDeclarationFragment fragment,
			List<VariableDeclarationFragment> fieldsToInitialize) {
	...
	private boolean areLocalsUsedIn(Expression fieldInitializer, List<VariableDeclarationFragment> fieldsToInitialize) {
		...
		for (IBinding iBinding : localsUsed) {
			...
			if (isBindingToTemp(curr) // Reference a local from outside
					|| (curr.isField() && curr.getDeclaringClass() == anonType
							&& fieldsToInitialize.contains(fCompilationUnitNode.findDeclaringNode(curr)))) { // References
																												// a
																												// field
																												// that
																												// references
	...
	private IMethodBinding getSuperConstructorBinding() {
		...
		IMethodBinding anonConstr = ((ClassInstanceCreation) fAnonymousInnerClassNode.getParent())
	...
	private SingleVariableDeclaration newParameterDeclaration(AST ast, ImportRewrite importRewrite, String paramName,
	...
	private void setSuperType(CompilationUnitRewrite rewrite, TypeDeclaration declaration,
			ITypeBinding[] impliedSuperTypeParameters) {
		ClassInstanceCreation classInstanceCreation = (ClassInstanceCreation) fAnonymousInnerClassNode.getParent();
...
//		System.out.println("fAnonymousInnerClassNode:" + fAnonymousInnerClassNode);
	...
	public boolean mustInnerClassBeStatic() {
		ITypeBinding typeBinding = ASTNodes.getParent(fAnonymousInnerClassNode, AbstractTypeDeclaration.class)
				.resolveBinding();
	...
	private RefactoringStatus initialize(JavaRefactoringArguments arguments) {
		...
		if (handle != null) {
			...
			if (element == null || !element.exists() || element.getElementType() != IJavaElement.COMPILATION_UNIT)
				return JavaRefactoringDescriptorUtil.createInputFatalStatus(element, getName(),
						IJavaRefactorings.CONVERT_ANONYMOUS);
			else {
		...
		if (declareStatic != null) {
			...
			return RefactoringStatus.createFatalErrorStatus(Messages
					.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_STATIC));
