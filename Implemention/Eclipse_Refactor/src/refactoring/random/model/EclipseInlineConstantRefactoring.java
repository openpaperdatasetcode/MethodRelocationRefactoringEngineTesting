...
public class EclipseInlineConstantRefactoring extends Refactoring {
	...
	private static class InlineTargetCompilationUnit {
		...
		private static class InitializerTraversal extends HierarchicalASTVisitor {
			...
			private static boolean areInSameType(ASTNode one, ASTNode other) {
				...
				if (one instanceof SimpleName) {
					...
					if (oneTypeBinding instanceof IVariableBinding) {
						ITypeBinding onesDeclaringClassBinding = ((IVariableBinding) oneTypeBinding)
								.getDeclaringClass();
			...
			public InitializerTraversal(Expression initializer, HashSet<SimpleName> staticImportsInInitializer,
					Expression newLocation, HashSet<SimpleName> staticImportsInReference,
				...
				fNewLocationContext = new ContextSensitiveImportRewriteContext(fNewLocation,
						fNewLocationCuRewrite.getImportRewrite());
			...
			private void addExplicitTypeArgumentsIfNecessary(Expression invocation) {
				if (Invocations.isResolvedTypeInferredFromExpectedType(invocation)) {
					...
					if (!(referenceContext instanceof VariableDeclarationFragment)
						...
						ListRewrite typeArgsRewrite = Invocations.getInferredTypeArgumentsRewrite(fInitializerRewrite,
								invocation);
						for (ITypeBinding typeArgument2 : Invocations.getInferredTypeArguments(invocation)) {
							Type typeArgument = fNewLocationCuRewrite.getImportRewrite().addImport(typeArgument2,
									fNewLocationCuRewrite.getAST(), fNewLocationContext, TypeLocation.TYPE_ARGUMENT);
						...
						if (invocation instanceof MethodInvocation) {
							...
							if (expression == null) {
								...
								if (methodBinding != null) {
									expression = fNewLocationCuRewrite.getAST()
											.newName(fNewLocationCuRewrite.getImportRewrite().addImport(
													methodBinding.getDeclaringClass().getTypeDeclaration(),
													fNewLocationContext));
									fInitializerRewrite.set(invocation, MethodInvocation.EXPRESSION_PROPERTY,
											expression, null);
								}
							}
						}
			...
			public boolean visit(Name name) {
				...
				if (leftmostBinding instanceof ITypeBinding) {
					String addedImport = fNewLocationCuRewrite.getImportRewrite()
			...
			private void qualifyMemberName(SimpleName memberName) {
				if (isStaticAccess(memberName)) {
					...
					if (memberBinding instanceof IVariableBinding || memberBinding instanceof IMethodBinding) {
						if (fStaticImportsInReference.contains(fNewLocation) // Use static import if reference location
																				// used static import
								|| fStaticImportsInInitializer2.contains(memberName)) { // Use static import if already
																						// imported statically in
																						// initializer
			...
			private void qualifyToTopLevelClass(SimpleName toQualify) {
				...
				Type newQualification = fNewLocationCuRewrite.getImportRewrite().addImport(declaringClass.getErasure(),
						fInitializerRewrite.getAST(), fNewLocationContext);
		...
		/**
		 * The references in this compilation unit, represented as AST Nodes in the
		 * parsed representation of the compilation unit
		 */
		...
		private InlineTargetCompilationUnit(CompilationUnitRewrite cuRewrite, Name[] references,
				EclipseInlineConstantRefactoring refactoring, HashSet<SimpleName> staticImportsInInitializer) {
			...
			if (refactoring.getRemoveDeclaration() && refactoring.getReplaceAllReferences()
		...
		private void inlineReference(Expression reference) throws CoreException {
			...
			TextEditGroup msg = fCuRewrite
					.createGroupDescription(RefactoringCoreMessages.InlineConstantRefactoring_Inline);
			...
			if (explicitCast != null) {
				...
				Expression modifiedInitializerExpr = (Expression) fCuRewrite.getASTRewrite()
						.createStringPlaceholder(modifiedInitializer, reference.getNodeType());
				if (NecessaryParenthesesChecker.needsParentheses(fInitializer, cast,
				...
				ImportRewriteContext context = new ContextSensitiveImportRewriteContext(reference,
						fCuRewrite.getImportRewrite());
				...
				ArrayInitializer newArrayInitializer = (ArrayInitializer) fCuRewrite.getASTRewrite()
				...
				newReference = (Expression) fCuRewrite.getASTRewrite().createStringPlaceholder(modifiedInitializer,
						reference.getNodeType());
				isStringPlaceholder = true;
			}

			if (NecessaryParenthesesChecker.needsParentheses((isStringPlaceholder ? fInitializer : newReference),
					reference.getParent(), reference.getLocationInParent())) {
				ParenthesizedExpression parenthesized = ast.newParenthesizedExpression();
		...
		private String prepareInitializerForLocation(Expression location) throws CoreException {
			...
			if (fIs15)
				ImportReferencesCollector.collect(location, project, null, new ArrayList<SimpleName>(),
						staticImportsInReference);

			InitializerTraversal traversal = new InitializerTraversal(fInitializer, fStaticImportsInInitializer,
					location, staticImportsInReference, fCuRewrite);
			ASTRewrite initializerRewrite = traversal.getInitializerRewrite();
			IDocument document = new Document(fInitializerUnit.getBuffer().getContents()); // could reuse document when
																							// generating and applying
																							// undo edits

			final RangeMarker marker = new RangeMarker(fInitializer.getStartPosition(), fInitializer.getLength());
			TextEdit[] rewriteEdits = initializerRewrite.rewriteAST(document, fInitializerUnit.getOptions(true))
					.removeChildren();
			...
			try {
				...
				int oldIndent = Strings.computeIndentUnits(document.get(region.getOffset(), region.getLength()),
						fInitializerUnit);
				return Strings.changeIndent(rewrittenInitializer, oldIndent, fInitializerUnit, "", //$NON-NLS-1$
		...
		private void removeConstantDeclarationIfNecessary() {
			...
			TextEditGroup msg = fCuRewrite
					.createGroupDescription(RefactoringCoreMessages.InlineConstantRefactoring_remove_declaration);
	 ...
	 */
	public EclipseInlineConstantRefactoring(IField field) {
	 ...
	 *                        by scripting
	 * @param node            the compilation unit node, or <code>null</code> if
	 *                        invoked by scripting
	 * @param selectionStart  the start of the selection
	 * @param selectionLength the length of the selection
	 */
	public EclipseInlineConstantRefactoring(ICompilationUnit unit, CompilationUnit node, IField field, SimpleName name,
		...
		fSelectedConstantName = name;
		fField = field;
		if (unit != null)
			initialize(unit, node);
	}

	public EclipseInlineConstantRefactoring(JavaRefactoringArguments arguments, RefactoringStatus status) {
	...
	public RefactoringStatus checkStaticFinalConstantNameSelected() {
		if (fSelectedConstantName == null)
			return RefactoringStatus.createStatus(RefactoringStatus.FATAL,
					RefactoringCoreMessages.InlineConstantRefactoring_static_final_field, null,
					CorextCore.getPluginId(), RefactoringStatusCodes.NOT_STATIC_FINAL_SELECTED, null);
	...
	public RefactoringStatus checkInitialConditions(IProgressMonitor pm) throws CoreException {
		try {
			...
			if (!fSelectionCu.isStructureKnown())
				return RefactoringStatus.createStatus(RefactoringStatus.FATAL,
						RefactoringCoreMessages.InlineConstantRefactoring_syntax_errors, null, CorextCore.getPluginId(),
...
//		System.out.println("field:" + fSelectedConstantName);
//		fField = (IField) ((IVariableBinding) fSelectedConstantName.resolveBinding()).getJavaElement();
		if (fField != null && !fField.exists())
			return RefactoringStatus.createStatus(RefactoringStatus.FATAL,
					RefactoringCoreMessages.InlineConstantRefactoring_local_anonymous_unsupported, null,
					CorextCore.getPluginId(), RefactoringStatusCodes.LOCAL_AND_ANONYMOUS_NOT_SUPPORTED, null);
...
//		System.out.println("fSelectedConstantName:" + fSelectedConstantName);
		...
		VariableDeclarationFragment declaration = (VariableDeclarationFragment) fSelectionCuRewrite.getRoot()
				.findDeclaringNode(fSelectedConstantName.resolveBinding());
		if (declaration != null) {
			fDeclarationCuRewrite = fSelectionCuRewrite;
		...
		if (fField.getCompilationUnit() == null)
			return RefactoringStatus.createStatus(RefactoringStatus.FATAL,
					RefactoringCoreMessages.InlineConstantRefactoring_binary_file, null, CorextCore.getPluginId(),
					RefactoringStatusCodes.DECLARED_IN_CLASSFILE, null);
	...
	private RefactoringStatus checkInitializer() {
		...
		if (initializer == null)
			return RefactoringStatus.createStatus(RefactoringStatus.FATAL,
					RefactoringCoreMessages.InlineConstantRefactoring_blank_finals, null, CorextCore.getPluginId(),
					RefactoringStatusCodes.CANNOT_INLINE_BLANK_FINAL, null);

		fInitializerAllStaticFinal = ConstantChecks.isStaticFinalConstant(
	...
	public RefactoringStatus checkFinalConditions(IProgressMonitor pm) throws CoreException {
		...
		try {
			...
			if (staticImportsInInitializer != null && getInitializer() != null && fField != null) {
				ImportReferencesCollector.collect(getInitializer(), fField.getJavaProject(), null,
						new ArrayList<SimpleName>(), staticImportsInInitializer);
			}
			if (getReplaceAllReferences()) {
				for (SearchResultGroup group : getReferences(pm, result)) {
					...
					InlineTargetCompilationUnit targetCompilationUnit = new InlineTargetCompilationUnit(cuRewrite,
							references, this, staticImportsInInitializer);
...
//				return result;
	...
	private Name[] extractReferenceNodes(SearchMatch[] searchResults, CompilationUnit cuNode) {
		...
		for (int i = 0; i < searchResults.length; i++)
			references[i] = (Name) NodeFinder.perform(cuNode, searchResults[i].getOffset(),
					searchResults[i].getLength());
		return references;
	}
	...
	public Change createChange(IProgressMonitor pm) throws CoreException {
		try {
			...
			final String description = Messages.format(
					...
					JavaElementLabelsCore.getElementLabel(fField, JavaElementLabelsCore.ALL_DEFAULT));
			final String header = Messages.format(
					RefactoringCoreMessages.InlineConstantRefactoring_descriptor_description,
					new String[] {
							JavaElementLabelsCore.getElementLabel(fField, JavaElementLabelsCore.ALL_FULLY_QUALIFIED),
							JavaElementLabelsCore.getElementLabel(fField.getParent(),
									JavaElementLabelsCore.ALL_FULLY_QUALIFIED) });
			final JDTRefactoringDescriptorComment comment = new JDTRefactoringDescriptorComment(project, this, header);
			comment.addSetting(Messages.format(RefactoringCoreMessages.InlineConstantRefactoring_original_pattern,
					JavaElementLabelsCore.getElementLabel(fField, JavaElementLabelsCore.ALL_FULLY_QUALIFIED)));
			...
			final InlineConstantDescriptor descriptor = RefactoringSignatureDescriptorFactory
					.createInlineConstantDescriptor(project, description, comment.asString(), arguments, flags);
			...
			return new DynamicValidationRefactoringChange(descriptor,
					RefactoringCoreMessages.InlineConstantRefactoring_inline, fChanges);
		} finally {
			pm.done();
			fChanges = null;
		}
	...
	private RefactoringStatus initialize(JavaRefactoringArguments arguments) {
		...
		if (remove != null)
			...
			return RefactoringStatus.createFatalErrorStatus(Messages
					.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_REMOVE));
		return new RefactoringStatus();
	}
}
