...
import refactoring.random.model.SourceAnalyzer.NameData;
...
public class SourceProvider {
	 ...
	 * @return <code>true</code> if variable is referenced by the method, otherwise
	 *         <code>false</code>
	 */
	...
	public ITypeBinding getReturnType() {
		System.out.println("fDeclaration:" + fDeclaration);
	...
	public boolean needsReturnedExpressionParenthesis(ASTNode parent, StructuralPropertyDescriptor locationInParent) {
		...
		if (last instanceof ReturnStatement) {
			return NecessaryParenthesesChecker.needsParentheses(((ReturnStatement) last).getExpression(), parent,
					locationInParent);
		}
		return false;
	}
	...
	public TextEdit getDeleteEdit() {
		...
		Map<String, String> options = fTypeRoot instanceof ICompilationUnit
				? ((ICompilationUnit) fTypeRoot).getOptions(true)
				: fTypeRoot.getJavaProject().getOptions(true);
...
//		updateStaticReferences(rewriter, context);
		...
		/// if (hasReturnValue()) {
		if (true) {
			if (context.callMode == ASTNode.RETURN_STATEMENT || context.callMode == ASTNode.YIELD_STATEMENT) {
...
//		}
//		System.out.println("iCompilationUnit:" + iCompilationUnit);
		fDocument = EclipseInlineMethodRefactoring.extractDocument(iCompilationUnit);
		System.out.println("fDocument:" + fDocument);
		MultiTextEdit root = new MultiTextEdit(0, fDocument.getLength());
		root.addChildren(markers);

		try {
			TextEditProcessor processor = new TextEditProcessor(fDocument, root,
	...
	private void replaceParameterWithExpression(ASTRewrite rewriter, CallContext context, ImportRewrite importRewrite)
...
//		System.out.println("context.arguments:" + context.arguments);
		...
		try {
//			System.out.println("iCompilationUnit:" + iCompilationUnit);
			ITextFileBuffer buffer = RefactoringFileBuffers.acquire(iCompilationUnit);
			for (int i = 0; i < arguments.length; i++) {
				...
				if (expression instanceof SimpleName) {
					...
					try {
						expressionString = buffer.getDocument().get(expression.getStartPosition(),
				...
				for (SimpleName simpleName : references) {
					...
					Expression newExpression = (Expression) rewriter.createStringPlaceholder(expressionString,
							expression.getNodeType());
					AST ast = rewriter.getAST();
					ITypeBinding explicitCast = ASTNodes.getExplicitCast(expression, (Expression) element);
					if (explicitCast != null) {
						CastExpression cast = ast.newCastExpression();
						if (NecessaryParenthesesChecker.needsParentheses(expression, cast,
						...
						ImportRewriteContext importRewriteContext = new ContextSensitiveImportRewriteContext(expression,
								importRewrite);
						cast.setType(
								importRewrite.addImport(explicitCast, ast, importRewriteContext, TypeLocation.CAST));
						expression = newExpression = cast;
					}
					if (NecessaryParenthesesChecker.needsParentheses(expression, element.getParent(),
							element.getLocationInParent())) {
						newExpression = createParenthesizedExpression(newExpression, ast);
					}
...
//			RefactoringFileBuffers.release(context.compilationUnit);
	...
	private void updateImplicitReceivers(ASTRewrite rewriter, CallContext context) {
		...
		for (Expression expression : implicitReceivers) {
			...
			ImportRewriteContext importRewriteContext = new ContextSensitiveImportRewriteContext(node,
					context.importer);
			if (node instanceof MethodInvocation) {
				final MethodInvocation inv = (MethodInvocation) node;
				rewriter.set(inv, MethodInvocation.EXPRESSION_PROPERTY, createReceiver(rewriter, context,
						(IMethodBinding) inv.getName().resolveBinding(), importRewriteContext), null);
			} else if (node instanceof ClassInstanceCreation) {
				final ClassInstanceCreation inst = (ClassInstanceCreation) node;
				rewriter.set(inst, ClassInstanceCreation.EXPRESSION_PROPERTY,
						createReceiver(rewriter, context, inst.resolveConstructorBinding(), importRewriteContext),
						null);
			} else if (node instanceof ThisExpression) {
				rewriter.replace(node, rewriter.createStringPlaceholder(context.receiver, ASTNode.METHOD_INVOCATION),
						null);
			} else if (node instanceof FieldAccess) {
				final FieldAccess access = (FieldAccess) node;
				rewriter.set(access, FieldAccess.EXPRESSION_PROPERTY,
						createReceiver(rewriter, context, access.resolveFieldBinding(), importRewriteContext), null);
	...
	private void rewriteReferences(ASTRewrite rewriter, ITypeBinding[] typeArguments,
			List<NameData> typeParameterReferences) {
	...
	private String[] getBlocks(RangeMarker[] markers) throws BadLocationException {
		...
		for (int i = 0; i < markers.length; i++) {
			...
			if (fMarkerMode == STATEMENT_MODE && lines.length == 2 && isSingleControlStatementWithoutBlock()) {
				lines[1] = cu != null ? CodeFormatterUtil.createIndentString(1, cu) + lines[1]
						: CodeFormatterUtil.createIndentString(1, project) + lines[1];
			}
