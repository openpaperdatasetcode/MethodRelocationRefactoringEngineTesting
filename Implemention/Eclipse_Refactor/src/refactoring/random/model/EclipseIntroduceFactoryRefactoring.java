...
public class EclipseIntroduceFactoryRefactoring extends Refactoring {

	private static final String ATTRIBUTE_PROTECT = "protect"; //$NON-NLS-1$

	/**
	 * The handle for the compilation unit holding the selection that was passed
	 * into this refactoring.
	 */
	private ICompilationUnit fCUHandle;

	/**
	 * The AST for the compilation unit holding the selection that was passed into
	 * this refactoring.
	 */
	private CompilationUnit fCU;

	/**
	 * Handle for compilation unit in which the factory method/class/interface will
	 * be generated.
	 */
	...
	/**
	 * An array of <code>SearchResultGroup</code>'s of all call sites that refer to
	 * the constructor signature in question.
	 */
	...
	/**
	 * An array containing the names of the constructor's formal arguments, if
	 * available, otherwise "arg1" ... "argN".
	 */
	private String[] fFormalArgNames = null;

	/**
	 * An array of <code>ITypeBinding</code>'s that describes the types of the
	...
	/**
	 * True iff there are call sites for the constructor to be encapsulated located
	 * in binary classes.
	 */
	...
	/**
	 * The fully qualified name of the factory class. This is only used if invoked
	 * from a refactoring script.
	 */
	...
	/**
	 * Creates a new <code>IntroduceFactoryRefactoring</code> with the given
	 * selection on the given compilation unit.
	 * 
	 * @param cu              the <code>ICompilationUnit</code> in which the user
	 *                        selection was made, or <code>null</code> if invoked
	 *                        from scripting
	 ...
	 */
	public EclipseIntroduceFactoryRefactoring(ICompilationUnit cu, int selectionStart, int selectionLength) {
	...
	public EclipseIntroduceFactoryRefactoring(JavaRefactoringArguments arguments, RefactoringStatus status) {
		this(null, 0, 0);
	 ...
	 * selection, if it is an entire constructor call or the class name portion of a
	 * constructor call or constructor declaration, or null otherwise.
	 * 
	...
	/**
	 * Determines what kind of AST node was selected, and returns an error status if
	 * the kind of node is inappropriate for this refactoring.
	 * 
	 * @param pm
	...
	/*
	 * @see
	 * org.eclipse.jdt.internal.corext.refactoring.base.Refactoring#checkActivation(
	...
	/**
	 * @param searchHits
	 * @return the set of compilation units that will be affected by this particular
	 *         invocation of this refactoring. This in general includes the class
	 *         containing the constructor in question, as well as all call sites to
	 *         the constructor.
	 */
	...
	/**
	 * @param groups
	 * @return an array of <code>SearchResultGroup</code>'s like the argument, but
	 *         omitting those groups that have no corresponding compilation unit
	 ...
	 * @param pm
	 * @param status
	 * @return an array of <code>SearchResultGroup</code>'s that identify the search
	 *         matches
	 * @throws JavaModelException
	 */
	private SearchResultGroup[] searchForCallsTo(IMethodBinding methodBinding, IProgressMonitor pm,
	 ...
	 * @param pm          an <code>IProgressMonitor</code> to use during this
	 *                    potentially lengthy operation
	 * @param status
	 * @return an array of <code>SearchResultGroup</code>'s identifying all calls to
	 *         the given constructor signature
	 * @throws JavaModelException
	 */
	private SearchResultGroup[] findAllCallsTo(IMethodBinding ctorBinding, IProgressMonitor pm,
			RefactoringStatus status) throws JavaModelException {
	...
	private IType findNonPrimaryType(String fullyQualifiedName, IProgressMonitor pm, RefactoringStatus status)
		...
		engine.setFiltering(true, true);
		engine.setScope(
				RefactoringScopeFactory.create(fCtorBinding.getDeclaringClass().getJavaElement().getJavaProject()));
		engine.setStatus(status);
		engine.searchPattern(new SubProgressMonitor(pm, 1));
	...
	/*
	 * @see
	 * org.eclipse.jdt.internal.corext.refactoring.base.Refactoring#checkInput(org.
	...
	public RefactoringStatus checkFinalConditions(IProgressMonitor pm) throws CoreException {
		try {
			...
			result.merge(
					Checks.validateModifiesFiles(ResourceUtil.getFiles(affectedFiles), getValidationContext(), pm));

			if (fCallSitesInBinaryUnits)
				result.merge(RefactoringStatus
						.createWarningStatus(RefactoringCoreMessages.IntroduceFactory_callSitesInBinaryClass));

			if (Modifier.isAbstract(fCtorBinding.getDeclaringClass().getModifiers())) {
				result.merge(
						RefactoringStatus.createWarningStatus(RefactoringCoreMessages.IntroduceFactory_abstractClass));
			}
	...
	/**
	 * @return an array containing the argument names for the constructor identified
	 *         by <code>fCtorBinding</code>, if available, or default names if
	 *         unavailable (e.g. if the constructor resides in a binary unit).
	 */
	 ...
	 * @throws CoreException if an exception occurs while accessing its
	...
	private MethodDeclaration createFactoryMethod(AST ast, IMethodBinding ctorBinding, ASTRewrite unitRewriter)
		...
		if (Modifier.isAbstract(declaringClass.getModifiers())) {
			...
			CodeGenerationSettings settings = JavaPreferencesSettings
					.getCodeGenerationSettings(fCUHandle.getJavaProject());
			ImportRewriteContext context = new ContextSensitiveImportRewriteContext(fFactoryCU, decl.getStartPosition(),
					fImportRewriter);
			for (IMethodBinding unImplementedMethod : getUnimplementedMethods(declaringClass)) {
				MethodDeclaration newMethodDecl = StubUtility2Core.createImplementationStub(fCUHandle, unitRewriter,
						fImportRewriter, context, unImplementedMethod, unImplementedMethod.getDeclaringClass(),
						settings, false, new NodeFinder(fFactoryCU, decl.getStartPosition(), 0).getCoveringNode());
	 ...
	 */
	private void setCtorTypeArguments(ClassInstanceCreation newCtorCall, String ctorTypeName,
			ITypeBinding[] ctorOwnerTypeParameters, AST ast) {
	 ...
	 * arguments. E.g., for constructor <code>Foo()</code> in
	 * <code>Foo&lt;T&gt;</code>, the factory method defines a method type parameter
	 * <code>&lt;T&gt;</code> and returns a <code>Foo&lt;T&gt;</code>.
	 * 
	 * @param newMethod               the method whose return type is to be set
	 * @param retTypeName             the simple name of the return type (without
	 *                                type parameters)
	 * @param ctorOwnerTypeParameters the formal type parameters of the type that
	 *                                the factory method instantiates (whose
	 *                                constructor is being encapsulated)
	 * @param ast                     utility object used to create AST nodes
	 */
	private void setMethodReturnType(MethodDeclaration newMethod, String retTypeName,
			ITypeBinding[] ctorOwnerTypeParameters, AST ast) {
	...
	/**
	 * Creates and adds the necessary argument declarations to the given factory
	 * method.<br>
	 * An argument is needed for each original constructor argument for which the
	 * evaluation of the actual arguments across all calls was not able to be pushed
	 * inside the factory method (e.g. arguments with side-effects, references to
	 * fields if the factory method is to be static or reside in a factory class, or
	 * arguments that varied across the set of constructor calls).<br>
	 * <code>fArgTypes</code> identifies such arguments by a <code>null</code>
	 * value.
	 * 
	 * @param ast       utility object used to create AST nodes
	...
	/**
	 * Copies the constructor's parent type's type parameters, if any, as method
	 * type parameters of the new static factory method. (Recall that static methods
	 * can't refer to type arguments of the enclosing class, since they have no
	 * instance to serve as a context.)<br>
	 * Makes sure to copy the bounds from the owning type, to ensure that the return
	 * type of the factory method satisfies the bounds of the type being
	 * instantiated.<br>
	 * E.g., for ctor Foo() in the type Foo<T extends Number>, be sure that the
	 * factory method is declared as<br>
	...
	/**
	 * @param argType
	 * @param extraDims number of extra array dimensions to add to the resulting
	 *                  type
	 * @param ast
	 * @return a Type that describes the given ITypeBinding. If the binding refers
	 *         to an object type, use the import rewriter to determine whether the
	 *         reference requires a new import, or instead needs to be
	 *         qualified.<br>
	 ...
	 * encapsulated inside the factory method, and associate the arguments with the
	 * given constructor call object.
	 * 
	 * @param ast         utility object used to create AST nodes
	 ...
	 *         visibility to <code>protected</code>. This takes into account the
	 *         user- requested mode and whether the constructor's compilation unit
	 *         is in source form.
	 */
	...
	/**
	 * Creates and adds the necessary change to make the constructor method
	 * protected.
	 * 
	 * @param unitAST
	 * @param unitRewriter
	...
	private boolean protectConstructor(CompilationUnit unitAST, ASTRewrite unitRewriter, TextEditGroup declGD) {
		...
		// No need to rewrite the modifiers if the visibility is what we already want it
		// to be.
	 ...
	 * <code>SearchResultGroup</code> to implement the refactoring transformation to
	 * the given <code>CompilationUnitChange</code>.
	 * 
	 * @param rg         the <code>SearchResultGroup</code> for which changes should
	 *                   be created
	 * @param unitHandle
	 * @param unitChange the CompilationUnitChange object for the compilation unit
	 *                   in question
...
//		ICompilationUnit	unitHandle= rg.getCompilationUnit();
	 ...
	 *         If this is the unit containing the selection or the unit in which the
	 *         factory is to reside, checks the appropriate field (<code>fCU</code>
	 *         or <code>fFactoryCU</code>, respectively) and initializes the field
	 *         with a new AST only if not already done.
	 */
	...
	/**
	 * Use the given <code>ASTRewrite</code> to replace direct calls to the
	 * constructor with calls to the newly-created factory method.
	 *
	 * @param rg           the <code>SearchResultGroup</code> indicating all of the
	...
	private boolean replaceConstructorCalls(SearchResultGroup rg, CompilationUnit unit, ASTRewrite unitRewriter,
		...
		/*
		 * Sort by descending offset, such that nested constructor calls are processed
		 * first. This is necessary, since they can only be moved into the factory
		 * method invocation after they have been rewritten.
		 */
	...
	/**
	 * Look "in the vicinity" of the given range to find the
	 * <code>ClassInstanceCreation</code> node that this search hit identified.
	 * Necessary because the <code>SearchEngine</code> doesn't always cough up text
	 * extents that <code>NodeFinder.perform()</code> agrees with.
	 * 
	 * @param start
	 * @param length
	 * @param unitAST
	 * @return return a {@link ClassInstanceCreation} or a {@link MethodRef} or
	 *         <code>null</code> if this is really a constructor->constructor call
	 *         (e.g. "this(...)")
	 * @throws CoreException
	 */
	private ASTNode getCtorCallAt(int start, int length, CompilationUnit unitAST) throws CoreException {
			...
			throw new CoreException(JavaUIStatus.createError(IStatus.ERROR,
					Messages.format(RefactoringCoreMessages.IntroduceFactory_noASTNodeForConstructorSearchHit,
							new Object[] { Integer.toString(start), Integer.toString(start + length),
									BasicElementLabels
											.getJavaCodeString(unitHandle.getSource().substring(start, start + length)),
		...
		if (node instanceof ClassInstanceCreation) {
			...
			// We seem to have been given a hit for an implicit call to the base-class
			// constructor.
			// Do nothing with this (implicit) call, but have to make sure we make the
			// derived class
			// doesn't lose access to the base-class constructor (so make it 'protected',
			// not 'private').
			fConstructorVisibility = Modifier.PROTECTED;
			return null;
	 ...
	 * to create the factory method. The method will reside on the type identified
	 * by <code>fFactoryOwningClass</code>.
	 *
	 * @param unitRewriter the ASTRewrite to be used
	 * @param unit         the <code>CompilationUnit</code> where factory method
	 *                     will be created
	 * @param gd           the <code>GroupDescription</code> to associate with the
	 *                     changes made
	 * @throws CoreException if an exception occurs while accessing its
	 *                       corresponding resource
	 */
	private void createFactoryChange(ASTRewrite unitRewriter, CompilationUnit unit, TextEditGroup gd)
			throws CoreException {
		// ================================================================================
		// First add the factory itself (method, class, and interface as needed/directed
		// by user)
		AST ast = unit.getAST();
		...
		unitRewriter.getListRewrite(factoryOwner, factoryOwner.getBodyDeclarationsProperty()).insertAt(fFactoryMethod,
				idx, gd);
	}
		...
		try {
			...
			final String header = Messages.format(RefactoringCoreMessages.IntroduceFactory_descriptor_description,
					new String[] { BasicElementLabels.getJavaElementName(fNewMethodName),
							BindingLabelProviderCore.getBindingLabel(binding,
									JavaElementLabelsCore.ALL_FULLY_QUALIFIED),
							BindingLabelProviderCore.getBindingLabel(fCtorBinding,
									JavaElementLabelsCore.ALL_FULLY_QUALIFIED) });
			...
			comment.addSetting(Messages.format(RefactoringCoreMessages.IntroduceFactoryRefactoring_original_pattern,
					BindingLabelProviderCore.getBindingLabel(fCtorBinding, JavaElementLabelsCore.ALL_FULLY_QUALIFIED)));
			...
			comment.addSetting(Messages.format(RefactoringCoreMessages.IntroduceFactoryRefactoring_owner_pattern,
					BindingLabelProviderCore.getBindingLabel(binding, JavaElementLabelsCore.ALL_FULLY_QUALIFIED)));
			...
			if (!hitInCtorClass && !fFactoryUnitHandle.equals(ASTCreator.getCu(fCtorOwningClass))) { // Handle
																										// constructor-owning
																										// class if no
																										// search hits
																										// there
	 ...
	 * Returns a <code>RefactoringStatus</code> that indicates whether the given
	 * name is valid for the new factory method.
	 * 
	 ...
	 *         <code>newMethodName</code> is available to use as the name of the new
	 *         factory method within the factory-owner class (either a to-be-
	 *         created factory class or the constructor-owning class, depending on
	 *         the user options).
	 */
	private RefactoringStatus isUniqueMethodName(String methodName) {
		...
		if (Bindings.findMethodInType(declaringClass, methodName, fCtorBinding.getParameterTypes()) != null) {
			String format = Messages.format(RefactoringCoreMessages.IntroduceFactory_duplicateMethodName,
	 ...
	 * 
	 * @param fullyQualifiedTypeName an <code>IType</code> referring to an existing
	 *                               class
	...
	public RefactoringStatus setFactoryClass(String fullyQualifiedTypeName) {
		...
		try {
			...
			if (factoryType.isAnnotation())
				return RefactoringStatus
						.createErrorStatus(RefactoringCoreMessages.IntroduceFactory_cantPutFactoryMethodOnAnnotation);
			if (factoryType.isInterface())
				return RefactoringStatus
						.createErrorStatus(RefactoringCoreMessages.IntroduceFactory_cantPutFactoryMethodOnInterface);
		...
		if (factoryType.isBinary())
			...
			try {
				...
				fFactoryOwningClass = ASTNodes.getParent(NodeFinder.perform(fFactoryCU, factoryType.getNameRange()),
	...
	private IType findFactoryClass(String fullyQualifiedTypeName) throws JavaModelException {
		...
		if (factoryType == null) // presumably a non-primary type; try the search engine
			factoryType = findNonPrimaryType(fullyQualifiedTypeName, new NullProgressMonitor(),
					new RefactoringStatus());
		return factoryType;
	}

	/**
	 * Returns the name of the class on which the generated factory method is to be
	 * placed.
	 * 
