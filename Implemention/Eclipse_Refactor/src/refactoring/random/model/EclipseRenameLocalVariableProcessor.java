...
public class EclipseRenameLocalVariableProcessor extends JavaRenameProcessor implements IReferenceUpdating {
	 ...
	 * 
	 * @param localVariable the local variable, or <code>null</code> if invoked by
	 *                      scripting
	 */
	public EclipseRenameLocalVariableProcessor(ILocalVariable localVariable) {
	...
	public EclipseRenameLocalVariableProcessor(JavaRefactoringArguments arguments, RefactoringStatus status) {
		this(null);
	 ...
	 */
	EclipseRenameLocalVariableProcessor(ILocalVariable localVariable, TextChangeManager manager, CompilationUnit node,
	...
	/*
	 * @see org.eclipse.jdt.internal.corext.refactoring.rename.JavaRenameProcessor#
	...
	/*
	 * @see org.eclipse.jdt.internal.corext.refactoring.tagging.IReferenceUpdating#
	 * setUpdateReferences(boolean)
	 */
	@Override
	...
	/*
	 * @see org.eclipse.jdt.internal.corext.refactoring.tagging.INameUpdating#
	...
	/*
	 * @see org.eclipse.jdt.internal.corext.refactoring.tagging.INameUpdating#
	 * setNewElementName(java.lang.String)
	 */
	@Override
	...
	public RefactoringStatus checkInitialConditions(IProgressMonitor pm) throws CoreException {
		...
		if (fTempDeclarationNode == null)
			// || fTempDeclarationNode.resolveBinding() == null
...
//				return result;
			createEdits();
			if (!fIsComposite) {
				LocalAnalyzePackage[] localAnalyzePackages = new RenameAnalyzeUtil.LocalAnalyzePackage[] {
						fLocalAnalyzePackage };
				result.merge(RenameAnalyzeUtil.analyzeLocalRenames(localAnalyzePackages, fChange, fCompilationUnitNode,
						true));
			}
			return result;
		} finally {
	...
	/*
	 * @see org.eclipse.jdt.internal.corext.refactoring.tagging.INameUpdating#
	 * checkNewElementName(java.lang.String)
	 */
	@Override
	public RefactoringStatus checkNewElementName(String newName) throws JavaModelException {
		...
		if (!Checks.startsWithLowerCase(newName))
			if (fIsComposite) {
				final String nameOfParent = JavaElementLabelsCore.getElementLabel(fLocalVariable.getParent(),
						JavaElementLabelsCore.ALL_DEFAULT);
				final String nameOfType = JavaElementLabelsCore.getElementLabel(
						fLocalVariable.getAncestor(IJavaElement.TYPE), JavaElementLabelsCore.ALL_DEFAULT);
				result.addWarning(Messages.format(RefactoringCoreMessages.RenameTempRefactoring_lowercase2,
						new String[] { BasicElementLabels.getJavaElementName(newName), nameOfParent, nameOfType }));
			} else {
	...
	private void createEdits() {
		...
		for (int i = 0; i < allRenameEdits.length; i++) {
			...
			fChange.addTextEditGroup(
					new TextEditGroup(RefactoringCoreMessages.RenameTempRefactoring_changeName, allRenameEdits[i]));
		}
	...
	private RenameJavaElementDescriptor createRefactoringDescriptor() {
		...
		final String header = Messages.format(
				...
				new String[] { BasicElementLabels.getJavaElementName(fCurrentName),
						JavaElementLabelsCore.getElementLabel(fLocalVariable.getParent(),
				...
				if (offset >= 0 && length >= 0) {
					try {
						...
						if (fLocalVariable == null)
							return JavaRefactoringDescriptorUtil.createInputFatalStatus(null, getProcessorName(),
									IJavaRefactorings.RENAME_LOCAL_VARIABLE);
