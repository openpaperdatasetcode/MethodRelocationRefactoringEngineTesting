...
public class EclipsePromoteTempToFieldRefactoring extends Refactoring {
	 ...
	 */
	public EclipsePromoteTempToFieldRefactoring(ICompilationUnit unit, int selectionStart, int selectionLength) {
	 ...
	 */
	public EclipsePromoteTempToFieldRefactoring(VariableDeclaration declaration) {
	...
	public EclipsePromoteTempToFieldRefactoring(JavaRefactoringArguments arguments, RefactoringStatus status) {
		this(null);
...
//			return result;
		...
		if (isMethodParameter())
			return RefactoringStatus
					.createFatalErrorStatus(RefactoringCoreMessages.PromoteTempToFieldRefactoring_method_parameters);

		if (isTempAnExceptionInCatchBlock())
			return RefactoringStatus
					.createFatalErrorStatus(RefactoringCoreMessages.PromoteTempToFieldRefactoring_exceptions);
		...
		if (declaringType instanceof TypeDeclaration && ((TypeDeclaration) declaringType).isInterface())
			return RefactoringStatus
					.createFatalErrorStatus(RefactoringCoreMessages.PromoteTempToFieldRefactoring_interface_methods);
...
//			initializeDefaults();
	...
	public String[] guessFieldNames() {
		...
		return StubUtility.getFieldNameSuggestions(fCu.getJavaProject(), rawTempName, dim, getModifiers(),
				excludedNames);
	}
	...
	private void checkTempInitializerForLocalTypeUsage() {
		...
		ITypeBinding[] methodTypeParameters = declaringMethodBinding == null ? new ITypeBinding[0]
	...
	private RefactoringStatus checkTempTypeForLocalTypeUsage() {
		...
		if (vds == null)
			return RefactoringStatus
					.createFatalErrorStatus(RefactoringCoreMessages.PromoteTempToFieldRefactoring_cannot_promote);
	...
	private void initAST(IProgressMonitor pm) {
		if (fCompilationUnitNode == null) {
			...
			fTempDeclarationNode = TempDeclarationFinder.findTempDeclaration(fCompilationUnitNode, fSelectionStart,
					fSelectionLength);
		}
	}
		...
		if (declaration instanceof TypeDeclaration) {
			for (MethodDeclaration method : ((TypeDeclaration) declaration).getMethods()) {
				...
				if (names.contains(fFieldName)) {
					String[] keys = { BasicElementLabels.getJavaElementName(fFieldName), BindingLabelProviderCore
							.getBindingLabel(method.resolveBinding(), JavaElementLabelsCore.ALL_FULLY_QUALIFIED) };
					String msg = Messages.format(RefactoringCoreMessages.PromoteTempToFieldRefactoring_Name_conflict,
							keys);
	...
	private RefactoringStatus checkClashesWithExistingFields() {
		for (FieldDeclaration declaration : getFieldDeclarations()) {
			VariableDeclarationFragment[] fragments = (VariableDeclarationFragment[]) declaration.fragments()
			...
			for (VariableDeclarationFragment fragment : fragments) {
				if (fFieldName.equals(fragment.getName().getIdentifier())) {
					...
					return RefactoringStatus.createFatalErrorStatus(
							RefactoringCoreMessages.PromoteTempToFieldRefactoring_Name_conflict_with_field, context);
				}
			}
		}
		return null;
	...
	public Change createChange(IProgressMonitor pm) throws CoreException {
		...
		try {
			...
			TextChangeCompatibility.addTextEdit(result, RefactoringCoreMessages.PromoteTempToFieldRefactoring_editName,
					resultingEdits);
			return result;
	...
	private void addInitializersToConstructors(ASTRewrite rewrite) throws CoreException {
		...
		if (constructors.length == 0) {
			...
			newConstructor.modifiers()
					.addAll(ast.newModifiers(declaration.getModifiers() & ModifierRewrite.VISIBILITY_MODIFIERS));
	...
	private Javadoc getNewConstructorComment(ASTRewrite rewrite) throws CoreException {
		if (StubUtility.doAddComments(fCu.getJavaProject())) {
			String comment = CodeGeneration.getMethodComment(fCu, getEnclosingTypeName(), getEnclosingTypeName(),
					new String[0], new String[0], null, null, StubUtility.getLineDelimiterUsed(fCu));
	...
	private ConvertLocalVariableDescriptor getRefactoringDescriptor() {
		...
		final String header = Messages.format(
				...
				new String[] {
						...
						BindingLabelProviderCore.getBindingLabel(binding.getDeclaringMethod(),
		...
		comment.addSetting(
				Messages.format(RefactoringCoreMessages.PromoteTempToFieldRefactoring_visibility_pattern, visibility));
		...
		final ConvertLocalVariableDescriptor descriptor = RefactoringSignatureDescriptorFactory
				.createConvertLocalVariableDescriptor(project, description, comment.asString(), arguments,
	...
	private void addLocalDeclarationSplit(ASTRewrite rewrite) {
		...
		if (parentStatement instanceof SwitchStatement) {
			...
			// should not happen. VariableDeclaration's can not be in a control statement
			// body
			listRewrite = null;
			Assert.isTrue(false);
		}
		...
		if (fragmentIndex + 1 < fragments.size()) {
			...
			VariableDeclarationFragment copyfirstFragmentAfter = (VariableDeclarationFragment) rewrite
					.createCopyTarget(firstFragmentAfter);
			...
			if (modifiers.size() > 0) {
				ListRewrite modifiersRewrite = rewrite.getListRewrite(tempDeclarationStatement,
			...
			for (int i = fragmentIndex + 2; i < fragments.size(); i++) {
				...
				VariableDeclarationFragment fragmentCopy = (VariableDeclarationFragment) rewrite
	...
	private FieldDeclaration createNewFieldDeclaration(ASTRewrite rewrite) {
		...
		List<Dimension> extraDimensions = DimensionRewrite.copyDimensions(fTempDeclarationNode.extraDimensions(),
				rewrite);
		...
		if (ASTNodes.isVarType(vds, fCompilationUnitNode) && binding != null) {
			...
			ImportRewriteContext context = new ContextSensitiveImportRewriteContext(fCompilationUnitNode,
					importRewrite);
