...
public class AccessAnalyzer extends ASTVisitor {
	...
	public AccessAnalyzer(SelfEncapsulateFieldRefactoring refactoring, ICompilationUnit unit, IVariableBinding field,
			ITypeBinding declaringClass, ASTRewrite rewriter, ImportRewrite importRewrite) {
	...
	public boolean visit(Assignment node) {
		...
		if (!fIsFieldFinal) {
			...
			if (node.getOperator() == Assignment.Operator.ASSIGN) {
				...
				Expression rhs = (Expression) fRewriter.createCopyTarget(rightHandSide);
				if (NecessaryParenthesesChecker.needsParenthesesForRightOperand(rightHandSide, exp,
						leftHandSide.resolveTypeBinding())) {
	...
	public boolean visit(PrefixExpression node) {
		...
		fRewriter.replace(node, createInvocation(node.getAST(), node.getOperand(), node.getOperator().toString()),
	...
	public boolean visit(PostfixExpression node) {
		...
		if (!(parent instanceof ExpressionStatement)) {
			fStatus.addError(
					RefactoringCoreMessages.SelfEncapsulateField_AccessAnalyzer_cannot_convert_postfix_expression,
