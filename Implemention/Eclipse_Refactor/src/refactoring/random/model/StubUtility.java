...
public class StubUtility {
	 ...
	 */
	public static String getMethodBodyContent(boolean isConstructor, IJavaProject project, String destTypeName,
			String methodName, String bodyStatement, String lineDelimiter) throws CoreException {
		return getMethodBodyContent(false, isConstructor, project, destTypeName, methodName, bodyStatement,
				lineDelimiter);
	}

	public static String getMethodBodyContent(boolean useAlternativeMethodBody, boolean isConstructor,
			IJavaProject project, String destTypeName, String methodName, String bodyStatement, String lineDelimiter)
			throws CoreException {
		String templateName = isConstructor ? CodeTemplateContextType.CONSTRUCTORSTUB_ID
				: useAlternativeMethodBody ? "org.eclipse.jdt.ui.text.codetemplates.constructorbody"
	 ...
	 * Don't use this method directly, use CodeGeneration.
	 */
	public static String getGetterMethodBodyContent(IJavaProject project, String destTypeName, String methodName,
	 ...
	 * Don't use this method directly, use CodeGeneration.
	 */
	public static String getSetterMethodBodyContent(IJavaProject project, String destTypeName, String methodName,
	...
	public static String getCatchBodyContent(ICompilationUnit cu, String exceptionType, String variableName,
			ASTNode locationInAST, String lineDelimiter) throws CoreException {
	 ...
	 * Don't use this method directly, use CodeGeneration.
	 * 
	 * @see CodeGeneration#getCompilationUnitContent(ICompilationUnit, String,
	...
	public static String getCompilationUnitContent(ICompilationUnit cu, String packDecl, String fileComment,
	 ...
	 * Don't use this method directly, use CodeGeneration.
	 * 
	 * @see CodeGeneration#getTypeComment(ICompilationUnit, String, String[],
	 * String)
	 */
	public static String getTypeComment(ICompilationUnit cu, String typeQualifiedName, String[] typeParameterNames,
			String[] params, String lineDelim) throws CoreException {
		...
		TemplateVariable position = findVariable(buffer, CodeTemplateContextType.TAGS); // look if Javadoc tags have to
																						// be added
		if (position == null) {
			return str;
		}
		...
		for (int i = tagOffsets.length - 1; i >= 0; i--) { // from last to first
			try {
				insertTag(document, tagOffsets[i], position.getLength(), params, EMPTY, null, typeParameterNames, false,
						lineDelim);
			} catch (BadLocationException e) {
				throw new CoreException(
						new Status(IStatus.ERROR, JavaManipulation.ID_PLUGIN, IStatus.ERROR, e.getMessage(), e));
			}
		}
		return document.get();
	}
	 ...
	 * @throws CoreException thrown if the template could not be evaluated See
	 *                       CodeGeneration#getTypeBody(String, ICompilationUnit,
	 *                       String, String)
	 */
	public static String getTypeBody(String templateID, ICompilationUnit cu, String typeName, String lineDelim)
			throws CoreException {
	 ...
	 * Don't use this method directly, use CodeGeneration.
	 * 
	 * @see CodeGeneration#getMethodComment(ICompilationUnit, String, String,
	 * String[], String[], String, String[], IMethod, String)
	 */
	 ...
	 * Don't use this method directly, use CodeGeneration.
	 * 
	 * @see CodeGeneration#getModuleComment(IJavaProject, String, String, String[],
	 ...
	 * Don't use this method directly, use CodeGeneration.
	 * 
	 * @see CodeGeneration#getSetterComment(ICompilationUnit, String, String,
	 * String, String, String, String, String)
	 */
	public static String getSetterComment(ICompilationUnit cu, String typeName, String methodName, String fieldName,
	 ...
	 * Don't use this method directly, use CodeGeneration.
	 * 
	 * @see CodeGeneration#getGetterComment(ICompilationUnit, String, String,
	 * String, String, String, String)
	 */
	public static String getGetterComment(ICompilationUnit cu, String typeName, String methodName, String fieldName,
	...
	/*
	 * Don't use this method directly, use CodeGeneration. This method should work
	 * with all AST levels.
	 * 
	 * @see CodeGeneration#getMethodComment(ICompilationUnit, String,
	 * MethodDeclaration, boolean, String, String[], String)
	 */
	public static String getMethodComment(ICompilationUnit cu, String typeName, MethodDeclaration decl,
			boolean isDeprecated, String targetName, String targetMethodDeclaringTypeName,
	...
	public static boolean shouldGenerateMethodTypeParameterTags(IJavaProject project) {
		return JavaCore.ENABLED
				.equals(project.getOption(JavaCore.COMPILER_PB_MISSING_JAVADOC_TAGS_METHOD_TYPE_PARAMETERS, true));
	}
	...
	public static String[] getVariableNameSuggestions(int variableKind, IJavaProject project, ITypeBinding expectedType,
		...
		if (assignedExpression != null) {
			...
			if (nameFromExpression != null) {
				add(getVariableNameSuggestions(variableKind, project, nameFromExpression, 0, excluded, false), res); // pass
																														// 0
																														// as
																														// dimension,
																														// base
			...
			if (nameFromParent != null) {
				add(getVariableNameSuggestions(variableKind, project, nameFromParent, 0, excluded, false), res); // pass
																													// 0
																													// as
																													// dimension,
																													// base
	...
	public static String[] getVariableNameSuggestions(int variableKind, IJavaProject project, ITypeBinding expectedType,
			Expression assignedExpression, Collection<String> excluded, String usedNameForIdenticalExpressionInCu,
			Collection<String> usedNamesForIdenticalExpressionInMethod) {
		LinkedHashSet<String> res = new LinkedHashSet<>(); // avoid duplicates but keep order
		...
		if (assignedExpression != null) {
			if (isTypeAvailable) {
				if (assignedExpression instanceof MethodInvocation) {
					// if we have a method invocation, see if we can recycle some previously used
					// variable name that is assigned with the same expression.
					String recycledName = recycleNames(typeName, assignedExpression, excluded,
							usedNameForIdenticalExpressionInCu, usedNamesForIdenticalExpressionInMethod);
					if (recycledName != null) {
						add(getVariableNameSuggestions(variableKind, project, recycledName, 0, excluded, false), res); // pass
																														// 0
																														// as
																														// dimension,
																														// base
		...
		if (!name.toLowerCase().contains(typeName.toLowerCase())) {
			...
			if (arguments.size() > 1 || argumentTypes.contains(Integer.valueOf(ASTNode.METHOD_INVOCATION))) {
				if (usedNameForIdenticalExpressionInCu != null) {
					if (excluded.contains(usedNameForIdenticalExpressionInCu)
							|| usedNamesForIdenticalExpressionInMethod.contains(usedNameForIdenticalExpressionInCu))
						return null;
					else
	...
	public static String[] getVariableNameSuggestions(int variableKind, IJavaProject project, Type expectedType,
			Expression assignedExpression, Collection<String> excluded) {
		LinkedHashSet<String> res = new LinkedHashSet<>(); // avoid duplicates but keep order
	...
	private static String[] getVariableNameSuggestions(int variableKind, IJavaProject project, Type expectedType,
	...
	/**
	 * Returns variable name suggestions for the given base name. This is a layer
	 * over the JDT.Core NamingConventions API to fix its shortcomings. JDT UI code
	 * should only use this API.
	 *
	 ...
	 *                        is expected to be a name without any pre- or suffixes
	 ...
	 * @param evaluateDefault if set, the result is guaranteed to contain at least
	 *                        one result. If not, the result can be an empty array.
	 *
	 * @return the name suggestions sorted by relevance (best proposal first). If
	 *         <code>evaluateDefault</code> is set to true, the returned array is
	 *         never empty. If <code>evaluateDefault</code> is set to false, an
	 *         empty array is returned if there is no good suggestion for the given
	 *         base name.
	 */
	...
	private static final String[] KNOWN_METHOD_NAME_PREFIXES = { "get", "is", "to", "create", "load", "find", //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$//$NON-NLS-4$//$NON-NLS-5$//$NON-NLS-6$
			"build", "generate", "prepare", "parse", "current", "read", "resolve", "retrieve", "make", "add", //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$//$NON-NLS-4$//$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$ //$NON-NLS-8$//$NON-NLS-9$//$NON-NLS-10$
			"extract" }; //$NON-NLS-1$
	...
	private static String getBaseNameFromExpression(IJavaProject project, Expression assignedExpression,
			int variableKind) {
		String name = null;
	...
	private static String getBaseNameFromReceiver(Expression receiver) {
		if (receiver != null) {
			if (receiver instanceof SimpleName) {
				...
				return receiverStr;
//						ConvertLoopOperation.modifyBaseName(receiverStr);
			}
		}
...
//			JavaManipulationPlugin.log(e);
	...
	public static ImportRewrite createImportRewrite(ICompilationUnit cu, boolean restoreExistingImports)
	 ...
	 * @param restoreExistingImports specifies if the existing imports should be
	 *                               kept or removed.
