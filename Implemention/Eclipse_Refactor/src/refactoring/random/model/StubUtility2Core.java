...
public final class StubUtility2Core {
	public static MethodDeclaration createImplementationStub(ICompilationUnit unit, ASTRewrite rewrite,
			ImportRewrite imports, ImportRewriteContext context, IMethodBinding binding, ITypeBinding targetType,
	...
	public static MethodDeclaration createImplementationStub(ICompilationUnit unit, ASTRewrite rewrite,
			ImportRewrite imports, ImportRewriteContext context, IMethodBinding binding, String[] parameterNames,
			ITypeBinding targetType, CodeGenerationSettings settings, boolean inInterface, ASTNode astNode)
		...
		if (astNode != null
			...
			nullnessDefault = RedundantNullnessTypeAnnotationsFilter.determineNonNullByDefaultLocations(astNode,
					RedundantNullnessTypeAnnotationsFilter.determineNonNullByDefaultNames(javaProject));

		MethodDeclaration decl = ast.newMethodDeclaration();
		decl.modifiers().addAll(StubUtility2Core.getImplementationModifiers(ast, binding, inInterface, imports, context,
				nullnessDefault));
		...
		List<SingleVariableDeclaration> parameters = StubUtility2Core.createParameters(javaProject, imports, context,
				ast, binding, parameterNames, decl, nullnessDefault);
		...
		if (!inInterface || (declaringType != typeObject && JavaModelUtil.is1d8OrHigher(javaProject))) {
			...
			if (Modifier.isAbstract(modifiers)) {
				Expression expression = ASTNodeFactory.newDefaultExpression(ast, decl.getReturnType2(),
						bindingReturnType, decl.getExtraDimensions());
				if (expression != null) {
				...
				if (declaringType.isInterface()) {
					ITypeBinding supertype = Bindings.findImmediateSuperTypeInHierarchy(targetType,
							declaringType.getTypeDeclaration().getQualifiedName());
			...
			String placeHolder = CodeGeneration.getMethodBodyContent(unit, type, binding.getName(), false,
					bodyStatement, delimiter);
			if (placeHolder != null) {
		...
		// According to JLS8 9.2, an interface doesn't implicitly declare non-public
		// members of Object,
		// and JLS8 9.6.4.4 doesn't allow @Override for these methods (clone and
		// finalize).
		boolean skipOverride = inInterface && declaringType == typeObject && !Modifier.isPublic(modifiers);

		if (!skipOverride) {
			StubUtility2Core.addOverrideAnnotation(settings, javaProject, rewrite, imports, decl,
					binding.getDeclaringClass().isInterface(), null);
		}
	...
	public static MethodDeclaration createConstructorStub(ICompilationUnit unit, ASTRewrite rewrite,
		...
		return StubUtility2Core.createConstructorStub(unit, rewrite, imports, context, binding, type, modifiers,
				omitSuperForDefConst, todo, settings,
				FormatterProfileManagerCore.getProjectSettings(unit.getJavaProject()));
	}

	/* This method should work with all AST levels. */
	public static MethodDeclaration createConstructorStub(ICompilationUnit unit, ASTRewrite rewrite,
			ImportRewrite imports, ImportRewriteContext context, IMethodBinding binding, String type, int modifiers,
			boolean omitSuperForDefConst, boolean todo, CodeGenerationSettings settings,
			Map<String, String> formatSettings) throws CoreException {
		AST ast = rewrite.getAST();
		MethodDeclaration decl = ast.newMethodDeclaration();
		...
		List<SingleVariableDeclaration> parameters = StubUtility2Core.createParameters(unit.getJavaProject(), imports,
				context, ast, binding, null, decl);
		...
		if (!omitSuperForDefConst || !parameters.isEmpty()) {
			...
			bodyStatement = ASTNodes.asFormattedString(invocation, 0, delimiter,
					formatSettings == null ? unit.getOptions(true) : formatSettings);
		}
	...
	public static MethodDeclaration createDelegationStub(ICompilationUnit unit, ASTRewrite rewrite,
			ImportRewrite imports, ImportRewriteContext context, IMethodBinding delegate,
			IVariableBinding delegatingField, CodeGenerationSettings settings) throws CoreException {
		...
		MethodDeclaration decl = ast.newMethodDeclaration();
		decl.modifiers().addAll(ASTNodeFactory.newModifiers(ast, delegate.getModifiers() & ~Modifier.DEFAULT
				& ~Modifier.SYNCHRONIZED & ~Modifier.ABSTRACT & ~Modifier.NATIVE));
		...
		List<SingleVariableDeclaration> params = createParameters(unit.getJavaProject(), imports, context, ast,
				delegate, null, decl);
		...
		if (settings.createComments) {
			/*
			 * TODO: have API for delegate method comments This is an inlined version of
			 * {@link CodeGeneration#getMethodComment(ICompilationUnit, String,
			 * MethodDeclaration, IMethodBinding, String)}
			 */
			...
			String string = StubUtility.getMethodComment(unit, qualifiedName, decl, delegate.isDeprecated(),
					linkToMethodName, declaringClassQualifiedName, parameterTypesQualifiedNames, true, delimiter);
			if (string != null) {
				Javadoc javadoc = (Javadoc) rewrite.createStringPlaceholder(string, ASTNode.JAVADOC);
				decl.setJavadoc(javadoc);
			}
	...
	public static MethodDeclaration createImplementationStubCore(ICompilationUnit unit, ASTRewrite rewrite,
			ImportRewrite imports, ImportRewriteContext context, IMethodBinding binding, ITypeBinding targetType,
			CodeGenerationSettings settings, boolean inInterface, ASTNode astNode, boolean snippetStringSupport)
			throws CoreException {
	...
	public static MethodDeclaration createImplementationStubCore(ICompilationUnit unit, ASTRewrite rewrite,
			ImportRewrite imports, ImportRewriteContext context, IMethodBinding binding, String[] parameterNames,
			ITypeBinding targetType, CodeGenerationSettings settings, boolean inInterface, ASTNode astNode,
			boolean snippetStringSupport) throws CoreException {
		return createImplementationStubCore(unit, rewrite, imports, context, binding, parameterNames, targetType,
				settings, inInterface, false, astNode, snippetStringSupport);
	}

	public static MethodDeclaration createImplementationStubCore(ICompilationUnit unit, ASTRewrite rewrite,
			ImportRewrite imports, ImportRewriteContext context, IMethodBinding binding, String[] parameterNames,
			ITypeBinding targetType, CodeGenerationSettings settings, boolean inInterface,
			boolean useAlternativeMethodBody, ASTNode astNode, boolean snippetStringSupport) throws CoreException {
		Assert.isNotNull(imports);
		Assert.isNotNull(rewrite);
		...
		if (!inInterface || (declaringType != typeObject && JavaModelUtil.is1d8OrHigher(javaProject))) {
			...
			if (bodyStatement != null) {
				...
				String bodyContent = CodeGeneration.getMethodBodyContent(unit, type, binding.getName(), false,
						useAlternativeMethodBody, bodyStatement, delimiter);
	...
	public static void createTypeParameters(ImportRewrite imports, ImportRewriteContext context, AST ast,
	...
	public static List<SingleVariableDeclaration> createParameters(IJavaProject project, ImportRewrite imports,
	...
	public static List<SingleVariableDeclaration> createParameters(IJavaProject project, ImportRewrite imports,
		...
		for (int i = 0; i < params.length; i++) {
			...
			if (!is50OrHigher) {
				...
				var.setVarargs(true);
				/*
				 * Varargs annotations are special. Example: foo(@O Object @A [] @B ... arg)
				 * => @B is not an annotation on the array dimension that constitutes the
				 * vararg. It's the type annotation of the *innermost* array dimension.
				 */
			...
			for (IAnnotationBinding annotation : annotations) {
				if (StubUtility2Core.isCopyOnInheritAnnotation(annotation.getAnnotationType(), project, nullnessDefault,
	...
	public static void createThrownExceptions(MethodDeclaration decl, IMethodBinding method, ImportRewrite imports,
	...
	public static List<IExtendedModifier> getImplementationModifiers(AST ast, IMethodBinding method,
			boolean inInterface, ImportRewrite importRewrite, ImportRewriteContext context,
			EnumSet<TypeLocation> nullnessDefault) throws JavaModelException {
					...
					for (IExtendedModifier otherModifier : otherModifiers) {
						if (otherModifier instanceof Modifier) {
							...
							for (IAnnotationBinding annotation : annotations) {
								...
								if (qn.endsWith(n) && (qn.length() == n.length()
										|| qn.charAt(qn.length() - n.length() - 1) == '.')) {
									if (StubUtility2Core.isCopyOnInheritAnnotation(otherAnnotationType, javaProject,
											nullnessDefault, TypeLocation.RETURN_TYPE))
		...
		for (IAnnotationBinding annotation : annotations) {
			if (StubUtility2Core.isCopyOnInheritAnnotation(annotation.getAnnotationType(), javaProject, nullnessDefault,
	...
	private static IMethodBinding[] getDelegateCandidates(ITypeBinding binding, ITypeBinding hierarchy) {
		...
		for (IMethodBinding typeMethod : binding.getDeclaredMethods()) {
			...
			if (!typeMethod.isConstructor() && !Modifier.isStatic(modifiers)
				...
				if (result != null && Flags.isFinal(result.getModifiers()) && !result.isSyntheticRecordMethod())
					continue;
	...
	public static IMethodBinding[] getOverridableMethods(AST ast, ITypeBinding typeBinding, boolean isSubType) {
		...
		for (IMethodBinding typeMethod : typeMethods) {
			...
			if (!typeMethod.isConstructor() && !Modifier.isStatic(modifiers) && !Modifier.isPrivate(modifiers)
					&& !typeMethod.isSyntheticRecordMethod()) {
				allMethods.add(typeMethod);
			}
		}
	...
	@SuppressWarnings("unchecked")
	public static Collection<IMethodBinding> getMethodsIn(ITypeBinding type,
			Predicate<IMethodBinding> ignoreTheseMethods) {
		if (type == null) {
		...
		for (IMethodBinding method : declaredMethods) {
			if (!isStatic(method) && !method.isConstructor() && !isPrivate(method)
					&& !ignoreTheseMethods.test(method)) {
				allMethods.add(method);
			}
		}
		...
		if (!type.isInterface()) {
			...
			for (IMethodBinding method : superMethods) {
				if (isConcrete(method) && Bindings.isVisibleInHierarchy(method, type.getPackage())
						&& findSubSignatureMethod(method, allMethods) == null) {
					allMethods.add(method);
				}
			}
		} else {
					...
					if (previouslyFound == null) {
						...
						if (superSigMethod != null && method.overrides(superSigMethod)) {
							...
							if (subSigMethod != null && superSigMethod != null) {
								if (!subSigMethod.getReturnType().getErasure()
										.isSubTypeCompatible(method.getReturnType().getErasure())) {
	...
	public static Predicate<IMethodBinding> IMPLEMENT_RECORD_SYNTHETICS = (IMethodBinding m) -> m
	...
	public static IMethodBinding[] getVisibleConstructors(ITypeBinding binding, boolean accountExisting,
		...
		if (constructorMethods.isEmpty()) {
			...
			if (method != null) {
				if ((proposeDefault || !accountExisting || existingConstructors == null
	 ...
	 *                                 <code>null</code>
	 * @param project                  the Java project used to access the compiler
	 *                                 settings
	 * @param rewrite                  the ASTRewrite
	 * @param imports                  the ImportRewrite
	 * @param methodDecl               the method declaration to add the annotation
	 *                                 to
	 * @param isDeclaringTypeInterface <code>true</code> if the type declaring the
	 *                                 method overridden by <code>methodDecl</code>
	 *                                 is an interface
