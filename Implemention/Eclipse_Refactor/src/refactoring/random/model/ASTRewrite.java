...
public class ASTRewrite {
	...
	/**
	 * Primary field used in representing rewrite properties efficiently. If
	 * <code>null</code>, this rewrite has no properties. If a {@link String}, this
	 * is the name of this rewrite's sole property, and <code>property2</code>
	 * contains its value. If a {@link Map}, this is the table of property
	 * name-value mappings. Initially <code>null</code>.
	 *
	 * @see #property2
	 */
	 ...
	 *                 <code>String</code>; value type: <code>String</code>); or
	 *                 <code>null</code> to use the standard global options
	 ...
	 * @throws IllegalArgumentException An <code>IllegalArgumentException</code> is
	 *                                  thrown if the document passed does not
	 ...
	 * {@link ITypeRoot} from which the AST was created from. The type root's source
	 * itself is not modified by this method call.
	 * <p>
	 * Important: This API can only be used if the modified AST has been created
	 * from a {@link ITypeRoot} with source. That means
	 * {@link ASTParser#setSource(ICompilationUnit)},
	 ...
	 *                                  the underlying compilation units buffer
	 *                                  could not be accessed.
	 * @throws IllegalArgumentException An {@link IllegalArgumentException} is
	 *                                  thrown if the document passed does not
	 *                                  correspond to the AST that is rewritten.
	 *
	 * @since 3.2
	...
	private TextEdit internalRewriteAST(char[] content, LineInformation lineInfo, String lineDelim, List commentNodes,
		...
		ASTRewriteAnalyzer visitor = new ASTRewriteAnalyzer(content, lineInfo, lineDelim, result, this.eventStore,
				this.nodeStore, commentNodes, options, sourceRangeComputer, recoveryScannerData);
	 ...
	 * IllegalArgumentException("The AST that is rewritten must not be modified.");
	 * //$NON-NLS-1$ } return true; } }; root.accept(isModifiedVisitor); }
	 */
	 ...
	 *
	 * @param node      the node being removed. The node can either be an original
	 *                  node in the AST or (since 3.4) a new node already inserted
	 *                  or used as replacement in this AST rewriter.
	 * @param editGroup the edit group in which to collect the corresponding text
	 *                  edits, or <code>null</code> if ungrouped
	 * @throws IllegalArgumentException if the node is null, or if the node is not
	 *                                  part of this rewriter's AST, or if the
	 *                                  described modification is invalid (such as
	 *                                  removing a required node)
	 */
	 ...
	 *
	 * @param node        the node being replaced. The node can either be an
	 *                    original node in the AST or (since 3.4) a new node already
	 ...
	 * @throws IllegalArgumentException if the node is null, or if the node is not
	 *                                  part of this rewriter's AST, or if the
	 *                                  replacement node is not a new node (or
	 *                                  placeholder), or if the described
	 ...
	 *
	 * @param node      the node
	 * @param property  the node's property; either a simple property or a child
	 *                  property
	 * @param value     the replacement child or new value, or <code>null</code> if
	 *                  none
	 * @param editGroup the edit group in which to collect the corresponding text
	 *                  edits, or <code>null</code> if ungrouped
	 * @throws IllegalArgumentException if the node or property is null, or if the
	 *                                  node is not part of this rewriter's AST, or
	 *                                  if the property is not a node property, or
	 *                                  if the described modification is invalid
	 */
	...
	/**
	 * Returns the value of the named property of this rewrite, or <code>null</code>
	 * if none.
	 *
	 * @param propertyName the property name
	 * @return the property value, or <code>null</code> if none
	 * @see #setProperty(String,Object)
	 * @throws IllegalArgumentException if the given property name is
	 *                                  <code>null</code>
	 * @since 3.7
	 */
	 ...
	 * @throws IllegalArgumentException if the node is null, or if the node is not
	 *                                  part of this rewriter's AST, or if the node
	...
	private void validatePropertyType(StructuralPropertyDescriptor prop, Object value) {
		...
		if (value == null) {
			...
			if (!valueType.isAssignableFrom(value.getClass())) {
				String message = value.getClass().getName() + " is not a valid type for " //$NON-NLS-1$
						+ prop.getNodeClass().getName() + " property '" + prop.getId() + '\''; //$NON-NLS-1$
				throw new IllegalArgumentException(message);
			}
		}
	}
	 ...
	 *
	 * @param code     the string to be inserted; lines should should not have extra
	 *                 indentation
	 * @param nodeType the ASTNode type that corresponds to the passed code.
	 * @return the new placeholder node
	 * @throws IllegalArgumentException if the code is null, or if the node type is
	 *                                  invalid
	 */
	 ...
	 * @throws IllegalArgumentException if the targetNodes is <code>null</code> or
	 *                                  empty
	 * @since 3.1
	 */
	 ...
	 * node. After obtaining a placeholder, the node must be removed or replaced.
	 * The placeholder node can either be inserted as new or used to replace an
	 * existing node. The placeholder must be used somewhere in the AST and must not
	 * be dropped by subsequent modifications. When the document is rewritten, the
	 * source code for the given node is inserted into the output document at the
	...
	/**
	 * Sets the named property of this rewrite to the given value, or to
	 ...
	 * properties on the same rewrite.
	 * </p>
	 * <p>
	 * Note that modifying a property is not considered a modification to the AST
	 * itself. This is to allow clients to decorate existing rewrites with their own
	 ...
	 *
	 * @param computer a target source range computer, or <code>null</code> to
