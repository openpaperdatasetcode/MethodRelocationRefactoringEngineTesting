...
class ExtractMethodAnalyzer extends CodeAnalyzer {
...
//			return result;
		...
		fEnclosingBodyDeclaration = EclipseExtractMethodRefactoring.methodDeclaration;
	...
	private void checkExpression(RefactoringStatus status) {
		...
		if (nodes != null && nodes.length == 1) {
			...
			if (node instanceof Type) {
				...
				status.addFatalError(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_switch_case,
						JavaStatusContext.create(fCUnit, node));
			} else if (node instanceof Annotation || ASTNodes.getParent(node, Annotation.class) != null) {
				status.addFatalError(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_from_annotation,
						JavaStatusContext.create(fCUnit, node));
			}
		}
	}
	...
	private void initReturnType(ImportRewrite rewriter) {
		...
		case ACCESS_TO_LOCAL:
			...
			fReturnType = ASTNodeFactory.newNonVarType(ast, declaration, rewriter,
		...
		case EXPRESSION:
			...
			if (fExpressionBinding != null) {
				if (fExpressionBinding.isNullType()) {
					getStatus().addFatalError(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_null_type,
					...
					if (normalizedBinding != null) {
						ImportRewriteContext context = new ContextSensitiveImportRewriteContext(
								fEnclosingBodyDeclaration, rewriter);
				...
				getStatus().addError(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_determine_return_type,
						JavaStatusContext.create(fCUnit, expression));
			}
			break;
			...
			astNode.accept(new ASTVisitor() {
				...
				public boolean visit(BreakStatement node) {
					...
					if (label != null && !fLocalLoopLabels.contains(label.getIdentifier())) {
						...
						ASTNode parentStatement = ASTNodes.getFirstAncestorOrNull(node, WhileStatement.class,
								ForStatement.class, DoStatement.class, SwitchStatement.class,
...
//		IRegion region = getSelectedNodeRange();
		start = EclipseExtractMethodRefactoring.fSelectionStart;
		length = EclipseExtractMethodRefactoring.fSelectionLength;
		...
		System.out.println("argOInfo:" + argInfo);
...
//		}
		...
		default:
			...
			for (int i = 0; i < localReads.size(); i++) {
				...
				String bindingName = BindingLabelProviderCore.getBindingLabel(binding,
						BindingLabelProviderCore.DEFAULT_TEXTFLAGS | JavaElementLabelsCore.F_PRE_TYPE_SIGNATURE);
...
//		for (IVariableBinding write : localInfo.get(flowContext,
//				FlowInfo.WRITE | FlowInfo.WRITE_POTENTIAL | FlowInfo.UNKNOWN)) {
...
//		}
	...
	private void adjustArgumentsAndMethodLocals() {
		for (int i = 0; i < fArguments.length; i++) {
			...
			// Both arguments and locals consider FlowInfo.WRITE_POTENTIAL. But at the end a
			// variable
			// can either be a local of an argument. Fix this based on the compute return
			// type which
	...
	private void computeExceptions() {
		fAllExceptions = EclipseExceptionAnalyzer.perform(getSelectedNodes());
	}
	...
	protected boolean handleSelectionEndsIn(ASTNode node) {
		invalidSelection(RefactoringCoreMessages.StatementAnalyzer_doesNotCover,
				JavaStatusContext.create(fCUnit, node));
				...
				if (coveringNode instanceof Block && coveringNode.getParent() instanceof MethodDeclaration) {
					...
					if (messages.length > 0) {
						status.addFatalError(
								Messages.format(RefactoringCoreMessages.ExtractMethodAnalyzer_compile_errors,
...
//				status.addFatalError(RefactoringCoreMessages.ExtractMethodAnalyzer_invalid_selection);
				...
				if (expression instanceof Name) {
					...
					if (name.resolveBinding() instanceof IVariableBinding) {
						...
						if ((isPartOfQualifiedName && !isPartOfQualifier)
								|| (locationInParent == FieldAccess.NAME_PROPERTY && !(((FieldAccess) name.getParent())
	...
	public boolean visit(AnonymousClassDeclaration node) {
		...
		if (isFirstSelectedNode(node)) {
			invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_anonymous_type,
					JavaStatusContext.create(fCUnit, node));
			return false;
		}
		return result;
	...
	public boolean visit(Assignment node) {
		...
		if ((selectedNode != null && SnippetFinder.isLeftHandSideOfAssignment(selectedNode))
				|| (selection.covers(node.getLeftHandSide()) && !selection.covers(node.getRightHandSide()))) {
			invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_leftHandSideOfAssignment,
					JavaStatusContext.create(fCUnit, node));
			return false;
		}
		return result;
	...
	public boolean visit(DoStatement node) {
		...
		try {
			...
			if (getSelection().getOffset() == actionStart) {
				invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_after_do_keyword,
						JavaStatusContext.create(fCUnit, getSelection()));
				return false;
			}
	...
	private boolean visitConstructorInvocation(ASTNode node, boolean superResult) {
		if (getSelection().getVisitSelectionMode(node) == Selection.SELECTED) {
			invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_super_or_this,
					JavaStatusContext.create(fCUnit, node));
			return false;
		}
		return superResult;
	...
	public boolean visit(VariableDeclarationFragment node) {
		...
		if (isFirstSelectedNode(node)) {
			if (node.getParent() instanceof FieldDeclaration) {
				invalidSelection(
						RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_variable_declaration_fragment_from_field,
						JavaStatusContext.create(fCUnit, node));
			} else {
				invalidSelection(
	...
	public void endVisit(ForStatement node) {
		if (getSelection().getEndVisitSelectionMode(node) == Selection.AFTER) {
			if (node.initializers().contains(getFirstSelectedNode())) {
				invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_for_initializer,
						JavaStatusContext.create(fCUnit, getSelection()));
			} else if (node.updaters().contains(getLastSelectedNode())) {
				invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_for_updater,
						JavaStatusContext.create(fCUnit, getSelection()));
			}
		}
		super.endVisit(node);
	...
	public void endVisit(QualifiedName node) {
		if (isResourceInTry(node)) {
			invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_resource_used_in_try_with_resources,
					JavaStatusContext.create(fCUnit, getSelection()));
		}
		super.endVisit(node);
	}
	...
	public void endVisit(BreakStatement node) {
		if (isFirstSelectedNode(node)) {
			invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_break,
					JavaStatusContext.create(fCUnit, getSelection()));
		}
	}
	...
	public void endVisit(ContinueStatement node) {
		if (isFirstSelectedNode(node)) {
			invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_continue,
					JavaStatusContext.create(fCUnit, getSelection()));
		}
	}
	...
	public void endVisit(YieldStatement node) {
		if (isFirstSelectedNode(node)) {
			invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_yield,
					JavaStatusContext.create(fCUnit, getSelection()));
		}
		super.endVisit(node);
	}
	...
	public void endVisit(VariableDeclarationExpression node) {
		if (isResourceInTry(node)) {
			invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_resource_in_try_with_resources,
					JavaStatusContext.create(fCUnit, getSelection()));
		}
	...
	private void checkTypeInDeclaration(Type node) {
		if (getSelection().getEndVisitSelectionMode(node) == Selection.SELECTED && getFirstSelectedNode() == node) {
			invalidSelection(RefactoringCoreMessages.ExtractMethodAnalyzer_cannot_extract_variable_declaration,
					JavaStatusContext.create(fCUnit, getSelection()));
		}
	}
	...
	private boolean isResourceInTry(Expression node) {
		return getSelection().getEndVisitSelectionMode(node) == Selection.SELECTED && getFirstSelectedNode() == node
				&& node.getLocationInParent() == TryStatement.RESOURCES2_PROPERTY;
	}
}
