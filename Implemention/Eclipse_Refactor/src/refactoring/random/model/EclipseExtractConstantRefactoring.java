...
public class EclipseExtractConstantRefactoring extends Refactoring {
	 ...
	 */
	public EclipseExtractConstantRefactoring(ICompilationUnit unit, int selectionStart, int selectionLength) {
	...
	public EclipseExtractConstantRefactoring(CompilationUnit astRoot, Type types, int selectionStart,
			int selectionLength) {
	...
	public EclipseExtractConstantRefactoring(JavaRefactoringArguments arguments, RefactoringStatus status) {
	...
	public void setVisibility(String am) {
		Assert.isTrue(am == JdtFlags.VISIBILITY_STRING_PRIVATE || am == JdtFlags.VISIBILITY_STRING_PROTECTED
				|| am == JdtFlags.VISIBILITY_STRING_PACKAGE || am == JdtFlags.VISIBILITY_STRING_PUBLIC);
		fVisibility = am;
	}
		...
		if (fGuessedConstNames == null) {
			try {
				...
				if (expression != null) {
					...
					fGuessedConstNames = StubUtility.getVariableNameSuggestions(NamingConventions.VK_STATIC_FINAL_FIELD,
	...
	private String[] getExcludedVariableNames() {
		if (fExcludedVariableNames == null) {
			try {
				...
				Collection<String> takenNames = new ScopeAnalyzer(fCuRewrite.getRoot())
...
//			System.out.println("2222");
			if (fCuRewrite == null) {
				CompilationUnit cuNode = RefactoringASTParser.parseWithASTProvider(fCu, true,
...
//					RefactoringCoreMessages.ExtractConstantRefactoring_select_expression, null,
//					JavaManipulationPlugin.getPluginId(), RefactoringStatusCodes.EXPRESSION_NOT_RVALUE, null);
		case Checks.NOT_RVALUE_VOID:
			return RefactoringStatus.createStatus(RefactoringStatus.FATAL,
					RefactoringCoreMessages.ExtractConstantRefactoring_no_void, null,
...
//			return result;
		...
		if (associatedExpression instanceof NullLiteral)
			result.merge(RefactoringStatus
					.createFatalErrorStatus(RefactoringCoreMessages.ExtractConstantRefactoring_null_literals));
...
//						.createFatalErrorStatus(RefactoringCoreMessages.ExtractConstantRefactoring_select_expression);
//		}
	...
	public RefactoringStatus checkConstantNameOnChange() throws JavaModelException {
		if (Arrays.asList(getExcludedVariableNames()).contains(fConstantName))
			return RefactoringStatus.createErrorStatus(
					Messages.format(RefactoringCoreMessages.ExtractConstantRefactoring_another_variable,
	...
	public RefactoringStatus checkFinalConditions(IProgressMonitor pm) throws CoreException {
		...
		/*
		 * Note: some checks are performed on change of input widget values. (e.g. see
		 * ExtractConstantRefactoring.checkConstantNameOnChange())
		 */
		...
		try {
			createConstantDeclaration();
			replaceExpressionsWithConstant();
			fChange = fCuRewrite.createChange(RefactoringCoreMessages.ExtractConstantRefactoring_change_name, true,
					new SubProgressMonitor(pm, 1));

			return fCheckResultForCompileProblems
					? RefactoringAnalyzeUtil.checkNewSource(fChange, fCu, fCuRewrite.getRoot(), pm)
...
//		Type type = getConstantType();
		Type type = ftypes;
		System.out.println("interface types:" + type);
		IExpressionFragment fragment = getSelectedExpression();
		Expression initializer = getSelectedExpression().createCopyTarget(fCuRewrite.getASTRewrite(), true);

		AST ast = fCuRewrite.getAST();
		VariableDeclarationFragment variableDeclarationFragment = ast.newVariableDeclarationFragment();
		fConstantName = fConstantName.replaceAll(Constant.regEx, "");
		variableDeclarationFragment.setName(ast.newSimpleName(fConstantName));
		variableDeclarationFragment.setInitializer(initializer);

		FieldDeclaration fieldDeclaration = ast.newFieldDeclaration(variableDeclarationFragment);
		if (type instanceof SimpleType) {
			fieldDeclaration.setType(ast.newSimpleType(ast.newSimpleName(type.toString())));
		} else if (type instanceof PrimitiveType) {
			fieldDeclaration.setType(ast.newParameterizedType(type));
		}
		...
		if (createComments) {
			String comment = CodeGeneration.getFieldComment(fCu, getConstantTypeName(), fConstantName,
		...
		ListRewrite listRewrite = fCuRewrite.getASTRewrite().getListRewrite(parent,
				parent.getBodyDeclarationsProperty());
		TextEditGroup msg = fCuRewrite
				.createGroupDescription(RefactoringCoreMessages.ExtractConstantRefactoring_declare_constant);
		if (insertFirst()) {
		...
		if (fLinkedProposalModel != null) {
			...
			ModifierCorrectionSubProcessorCore.installLinkedVisibilityProposals(fLinkedProposalModel, rewrite,
	...
	private Type getConstantType() throws JavaModelException {
		if (fConstantTypeCache == null) {
			...
			ImportRewriteContext context = new ContextSensitiveImportRewriteContext(fCuRewrite.getRoot(),
					fSelectionStart, importRewrite);
	...
	private ExtractConstantDescriptor createRefactoringDescriptor() {
		...
		final String header = Messages.format(RefactoringCoreMessages.ExtractConstantRefactoring_descriptor_description,
				new String[] { BasicElementLabels.getJavaElementName(fConstantName),
	...
	private void replaceExpressionsWithConstant() throws JavaModelException {
		...
		for (IASTFragment fragment : getFragmentsToReplace()) {
			...
			TextEditGroup description = fCuRewrite
					.createGroupDescription(RefactoringCoreMessages.ExtractConstantRefactoring_replace);
	...
	private void computeConstantDeclarationLocation() throws JavaModelException {
		...
		while (decls.hasNext()) {
			...
			if (decl instanceof FieldDeclaration)
				...
				continue; /*
							 * this declaration is not a field declaration or initializer, so the placement
							 * of the constant declaration relative to it does not matter
							 */
			}
	...
	private static boolean depends(IExpressionFragment selected, BodyDeclaration bd) {
		/*
		 * We currently consider selected to depend on bd only if db includes a
		 * declaration of a static field on which selected depends.
		 *
		 * A more accurate strategy might be to also check if bd contains (or is) a
		 * static initializer containing code which changes the value of a static field
		 * on which selected depends. However, if a static is written to multiple times
		 * within during class initialization, it is difficult to predict which value
		 * should be used. This would depend on which value is used by expressions
		 * instances for which the new constant will be substituted, and there may be
		 * many of these; in each, the static field in question may have taken on a
		 * different value (if some of these uses occur within static initializers).
		 */

		if (bd instanceof FieldDeclaration) {
			FieldDeclaration fieldDecl = (FieldDeclaration) bd;
			for (Iterator<VariableDeclarationFragment> fragments = fieldDecl.fragments().iterator(); fragments
					.hasNext();) {
				...
				if (selected.getSubFragmentsMatching(
						ASTFragmentFactory.createFragmentForFullSubtree(staticFieldName)).length != 0)
					return true;
			}
		}
		return false;
	...
	private Iterator<ASTNode> getReplacementScope() throws JavaModelException {
		...
		for (Iterator<BodyDeclaration> bodyDeclarations = containingType.bodyDeclarations().iterator(); bodyDeclarations
				.hasNext();) {
	...
	private IASTFragment[] getFragmentsToReplace() throws JavaModelException {
		...
		if (fReplaceAllOccurrences) {
			...
			while (replacementScope.hasNext()) {
				...
				IASTFragment[] allMatches = ASTFragmentFactory.createFragmentForFullSubtree(scope)
	...
	private AbstractTypeDeclaration getContainingTypeDeclarationNode() throws JavaModelException {
		AbstractTypeDeclaration result = ASTNodes.getParent(getSelectedExpression().getAssociatedNode(),
...
//		Assert.isNotNull(result);
	...
	private RefactoringStatus initialize(JavaRefactoringArguments arguments) {
		...
		if (handle != null) {
			...
			if (element == null || !element.exists() || element.getElementType() != IJavaElement.COMPILATION_UNIT)
				return JavaRefactoringDescriptorUtil.createInputFatalStatus(element, getName(),
						IJavaRefactorings.EXTRACT_CONSTANT);
			else
		...
		if (declareFinal != null) {
			...
			return RefactoringStatus.createFatalErrorStatus(Messages
					.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_QUALIFY));
		return new RefactoringStatus();
	}
}
