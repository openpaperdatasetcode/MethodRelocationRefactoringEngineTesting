...
import refactoring.model.MethodOccurenceCollector;

public class EclipseRenameFieldProcessor extends JavaRenameProcessor
	...
	private EclipseRenameLocalVariableProcessor fRenameLocalVariableProcessor;
	 ...
	 */
	public EclipseRenameFieldProcessor(IField field) {
	 ...
	 */
	public EclipseRenameFieldProcessor(JavaRefactoringArguments arguments, RefactoringStatus status) {
		this(null);
	 ...
	 */
	EclipseRenameFieldProcessor(IField field, TextChangeManager manager, GroupCategorySet categorySet) {
		initialize(field);
	...
	public RefactoringStatus checkNewElementName(String newName) throws CoreException {
		...
		if (isInstanceField(fField) && (!Checks.startsWithLowerCase(newName)))
			result.addWarning(
					fIsComposite
							? Messages.format(RefactoringCoreMessages.RenameFieldRefactoring_should_start_lowercase2,
									new String[] { BasicElementLabels.getJavaElementName(newName),
											getDeclaringTypeLabel() })
							: RefactoringCoreMessages.RenameFieldRefactoring_should_start_lowercase);

		if (Checks.isAlreadyNamed(fField, newName))
			result.addError(
					fIsComposite
							? Messages.format(RefactoringCoreMessages.RenameFieldRefactoring_another_name2,
									new String[] { BasicElementLabels.getJavaElementName(newName),
											getDeclaringTypeLabel() })
		...
		if (fField.getDeclaringType().getField(newName).exists())
			result.addError(
					fIsComposite
							? Messages.format(RefactoringCoreMessages.RenameFieldRefactoring_field_already_defined2,
									new String[] { BasicElementLabels.getJavaElementName(newName),
											getDeclaringTypeLabel() })
		...
		if (fIsRecordComponent && fField != null) {
			...
			try {
				if (parent != null && parent.isRecord()) {
					fCompUnit = SharedASTProviderCore.getAST(fField.getCompilationUnit(),
							SharedASTProviderCore.WAIT_YES, null);
					for (IJavaElement elem : parent.getChildren()) {
						if (elem instanceof IMethod && ((IMethod) elem).isConstructor()) {
								...
								if (mBinding != null && mBinding.isCanonicalConstructor()) {
									...
									for (ILocalVariable lVar : localVars) {
										if (lVar.getElementName().equals(fField.getElementName())) {
											...
											fRenameLocalVariableProcessor = createLocalRenameProcessor(lVar,
													getNewElementName(), fCompUnit);
										}
									}
								}
							}
	...
	public String getNewGetterName() throws CoreException {
		IMethod primaryGetterCandidate = JavaModelUtil.findMethod(GetterSetterUtil.getGetterName(fField, new String[0]),
				new String[0], false, fField.getDeclaringType());
		if (!JavaModelUtil.isBoolean(fField) || (primaryGetterCandidate != null && primaryGetterCandidate.exists()))
			return GetterSetterUtil.getGetterName(fField.getJavaProject(), getNewElementName(), fField.getFlags(),
					JavaModelUtil.isBoolean(fField), null);
		// bug 30906 describes why we need to look for other alternatives here
		return GetterSetterUtil.getGetterName(fField.getJavaProject(), getNewElementName(), fField.getFlags(), false,
				null);
	}

	public String getNewSetterName() throws CoreException {
		return GetterSetterUtil.getSetterName(fField.getJavaProject(), getNewElementName(), fField.getFlags(),
				JavaModelUtil.isBoolean(fField), null);
	}
	...
	/**
	 * Returns the maximum number of delegates which can be created for the input
	 * elements of this refactoring.
	 *
	...
	public int getDelegateCount() {
		int count = 0;
		try {
			if (isDelegateCreationAvailable(getField()))
				count++;
...
//				return result;
			...
			if (getGetter() != null && fRenameGetter) {
				...
				result.merge(Checks.checkIfConstructorName(getGetter(), getNewGetterName(),
			...
			if (getSetter() != null && fRenameSetter) {
				...
				result.merge(Checks.checkIfConstructorName(getSetter(), getNewSetterName(),
						fField.getDeclaringType().getElementName()));
			} else {
				pm.worked(1);
			}
			...
			if (fIsRecordComponent) {
				result.merge(checkRecordComponentAccessor(new SubProgressMonitor(pm, 1), getAccessor(),
						getNewElementName()));
				result.merge(Checks.checkIfConstructorName(getAccessor(), getNewElementName(),
						fField.getDeclaringType().getElementName()));
			} else {
				pm.worked(1);
			}
...
//					return result;
				result.merge(
						fRenameLocalVariableProcessor.checkFinalConditions(new SubProgressMonitor(pm, 1), context));
//				if (result.hasFatalError())
//					return result;
			}
...
//				return result;
	...
	private RefactoringStatus checkRecordComponentAccessor(IProgressMonitor pm, IMethod existingAccessor,
			String newAccessorName) throws CoreException {
		RefactoringStatus result = new RefactoringStatus();
	...
	private RefactoringStatus checkNewAccessor(IMethod existingAccessor, String newAccessorName) throws CoreException {
		RefactoringStatus result = new RefactoringStatus();
		IMethod accessor = JavaModelUtil.findMethod(newAccessorName, existingAccessor.getParameterTypes(), false,
		...
		String message = Messages.format(RefactoringCoreMessages.RenameFieldRefactoring_already_exists,
				new String[] { JavaElementUtil.createMethodSignature(accessor),
						BasicElementLabels.getJavaElementName(fField.getDeclaringType().getFullyQualifiedName('.')) });
	...
	private RefactoringStatus checkNewRecordComponentAccessor(String newAccessorName) throws CoreException {
		...
		String message = Messages.format(
				RefactoringCoreMessages.RenameFieldRefactoring_recordromponent_accessor_method_already_exists,
				new String[] { JavaElementUtil.createMethodSignature(accessor),
						BasicElementLabels.getJavaElementName(fField.getDeclaringType().getFullyQualifiedName('.')) });
		result.addError(message, JavaStatusContext.create(accessor));
		return result;
...
//		Assert.isTrue(groupDeclarations.length > 0);
		...
		for (IType nestedType : nestedTypes) {
			...
			if (otherField.exists()) {
				String msg = Messages.format(RefactoringCoreMessages.RenameFieldRefactoring_hiding,
						new String[] { BasicElementLabels.getJavaElementName(fField.getElementName()),
								BasicElementLabels.getJavaElementName(getNewElementName()),
								BasicElementLabels.getJavaElementName(nestedType.getFullyQualifiedName('.')) });
	...
	/*
	 * (non java-doc) Analyzes all compilation units in which type is referenced
	 */
...
//		result.merge(Checks.checkCompileErrorsInAffectedFiles(fReferences));
	 ...
	 * @param filterOut if <code>true</code>, filters out the references to record
	 *                  component accessor methods. If <code>false</code>, returns
	 *                  only the references to record component accessor methods.
		...
		if (this.fIsRecordComponent) {
			...
			for (SearchResultGroup g : grouped) {
				...
				if (newList.size() != matches.length) {
					result.add(
							new SearchResultGroup(g.getResource(), newList.toArray(new SearchMatch[newList.size()])));
				} else {
					result.add(g);
				}
			}
	...
	protected RenameJavaElementDescriptor createRefactoringDescriptor() {
		...
		final String header = Messages.format(RefactoringCoreMessages.RenameFieldProcessor_descriptor_description,
				new String[] {
						BasicElementLabels.getJavaElementName(fField.getElementName()), JavaElementLabelsCore
								.getElementLabel(fField.getParent(), JavaElementLabelsCore.ALL_FULLY_QUALIFIED),
						getNewElementName() });
	...
	private RefactoringStatus addDelegates() throws JavaModelException, CoreException {
		...
		if (isDelegateCreationAvailable(fField)) {
			...
			if (fieldDeclaration.fragments().size() > 1) {
				status.addWarning(Messages.format(
						RefactoringCoreMessages.DelegateCreator_cannot_create_field_delegate_more_than_one_fragment,
						BasicElementLabels.getJavaElementName(fField.getElementName())),
						JavaStatusContext.create(fField));
	...
	private void addGetterOccurrences(IProgressMonitor pm, RefactoringStatus status) throws CoreException {
		addAccessorOccurrences(pm, getGetter(), RefactoringCoreMessages.RenameFieldRefactoring_Update_getter_occurrence,
				getNewGetterName(), status);
	}

	private void addSetterOccurrences(IProgressMonitor pm, RefactoringStatus status) throws CoreException {
		addAccessorOccurrences(pm, getSetter(), RefactoringCoreMessages.RenameFieldRefactoring_Update_setter_occurrence,
				getNewSetterName(), status);
	}

	private void addAccessorOccurrences(IProgressMonitor pm, RefactoringStatus status) throws CoreException {
		if (getAccessor() != null) {
			addAccessorOccurrences(pm, getAccessor(),
					RefactoringCoreMessages.RenameFieldRefactoring_Update_getter_occurrence, getNewElementName(),
					status);
		} else {
			addFieldAccessorOccurrences(pm, RefactoringCoreMessages.RenameFieldRefactoring_Update_getter_occurrence,
					getNewElementName(), status);
		}
	}

	private void addAccessorOccurrences(IProgressMonitor pm, IMethod accessor, String editName, String newAccessorName,
		...
		SearchResultGroup[] groupedResults = RefactoringSearchEngine.search(pattern, scope,
				new MethodOccurenceCollector(accessor.getElementName()), pm, status);
	...
	private void addLocalVariableOccurrences(String newName, RefactoringStatus status) throws CoreException {
		...
		if (!fIsCompactConstructor) {
			status.merge(
					RenameAnalyzeUtil.analyzeLocalRenames(analyzePackages, fChangeManager.get(cu), fCompUnit, false));
		} else {
			status.merge(RenameAnalyzeUtil.analyzeCompactConstructorLocalRenames(analyzePackages,
					fChangeManager.get(cu), fCompUnit, false));
		}
	}

	private void addFieldAccessorOccurrences(IProgressMonitor pm, String editName, String newAccessorName,
			RefactoringStatus status) throws CoreException {
	...
	private RefactoringStatus analyzeRenameChanges(IProgressMonitor pm) throws CoreException {
		...
		try {
			...
			newWorkingCopies = RenameAnalyzeUtil.createNewWorkingCopies(
					compilationUnitsToModify.toArray(new ICompilationUnit[compilationUnitsToModify.size()]),
					fChangeManager, newWCOwner, new SubProgressMonitor(pm, 1));

			SearchResultGroup[] newReferences = getNewReferences(new SubProgressMonitor(pm, 1), result, newWCOwner,
					newWorkingCopies);
			result.merge(RenameAnalyzeUtil.analyzeRenameChanges2(fChangeManager, oldReferences, newReferences,
					getNewElementName()));
			return result;
		} finally {
			pm.done();
	...
	private SearchResultGroup[] getNewReferences(IProgressMonitor pm, RefactoringStatus status, WorkingCopyOwner owner,
			ICompilationUnit[] newWorkingCopies) throws CoreException {
		pm.beginTask("", 2); //$NON-NLS-1$
		ICompilationUnit declaringCuWorkingCopy = RenameAnalyzeUtil.findWorkingCopyForCu(newWorkingCopies,
		...
		if (fDelegateUpdating && isDelegateCreationAvailable(getField())) {
		...
		return RefactoringSearchEngine.search(newPattern, owner, scope, requestor, new SubProgressMonitor(pm, 1),
				status);
	}
