...
import refactoring.random.model.SnippetFinder.Match;

public class EclipseExtractMethodRefactoring extends Refactoring {
	...
	public static int fSelectionStart;
	public static int fSelectionLength;
	private AST fAST;
	...
	public List<Statement> extractList;
	 ...
	 * 
	 * @param unit            the compilation unit, or <code>null</code> if invoked
	 *                        by scripting
	 * @param selectionStart  selection start
	 * @param selectionLength selection end
	 */
	public EclipseExtractMethodRefactoring(ICompilationUnit unit, int selectionStart, int selectionLength) {
		fCUnit = unit;
		fRoot = null;
	...
	public EclipseExtractMethodRefactoring(JavaRefactoringArguments arguments, RefactoringStatus status) {
	 ...
	 */
	public EclipseExtractMethodRefactoring(CompilationUnit astRoot, MethodDeclaration mDeclaration, int selectionStart,
			int selectionLength, List<Statement> statements, Block block) {
		this((ICompilationUnit) astRoot.getTypeRoot(), selectionStart, selectionLength);
		fRoot = astRoot;
		methodDeclaration = mDeclaration;
		extractList = statements;
		methodBody = block;
	}
...
//			return result;
	...
	private ASTVisitor createVisitor() throws CoreException {
		fAnalyzer = new ExtractMethodAnalyzer(fCUnit,
				Selection.createFromStartLength(fSelectionStart, fSelectionLength));
		return fAnalyzer;
	}
	 ...
	 *
	 * @param throwRuntimeExceptions flag indicating if the new method throws
	 *                               runtime exceptions
	 */
		...
		for (ParameterInfo parameter : fParameterInfos) {
			...
			for (ParameterInfo other : fParameterInfos) {
				if (parameter != other && other.getNewName().equals(parameter.getNewName())) {
					result.addError(
							Messages.format(RefactoringCoreMessages.ExtractMethodRefactoring_error_sameParameter,
									BasicElementLabels.getJavaElementName(other.getNewName())));
					return result;
				}
			}
			if (parameter.isRenamed() && fUsedNames.contains(parameter.getNewName())) {
				result.addError(Messages.format(RefactoringCoreMessages.ExtractMethodRefactoring_error_nameInUse,
		...
		for (Iterator<ParameterInfo> iter = fParameterInfos.iterator(); iter.hasNext();) {
			...
			if (info.isOldVarargs() && iter.hasNext()) {
				return RefactoringStatus.createFatalErrorStatus(
						Messages.format(RefactoringCoreMessages.ExtractMethodRefactoring_error_vararg_ordering,
	...
	public RefactoringStatus checkFinalConditions(IProgressMonitor pm) throws CoreException {
		...
		BodyDeclaration node = methodDeclaration;
//				fAnalyzer.getEnclosingBodyDeclaration();
		if (node != null) {
//			System.out.println("result:" + result);
//			System.out.println("fMethodName:" + fMethodName);
//			System.out.println("fDestination:" + fDestination);
...
//			System.out.println("11111");
			ASTNode[] selectedNodes = getSelectNodes(methodDeclaration);
...
//					fCUnit.getBuffer(), fSelectionStart, fSelectionLength));

			TextEditGroup substituteDesc = new TextEditGroup(
					Messages.format(RefactoringCoreMessages.ExtractMethodRefactoring_substitute_with_call,
							BasicElementLabels.getJavaElementName(fMethodName)));
			result.addTextEditGroup(substituteDesc);

			MethodDeclaration mm;
			try {
				mm = createNewMethod(selectedNodes, fCUnit.findRecommendedLineSeparator(), substituteDesc);
				if (fLinkedProposalModel != null) {
					...
					ModifierCorrectionSubProcessorCore.installLinkedVisibilityProposals(fLinkedProposalModel, fRewriter,
							mm.modifiers(), false);
				}

				TextEditGroup insertDesc = new TextEditGroup(
						Messages.format(RefactoringCoreMessages.ExtractMethodRefactoring_add_method,
								BasicElementLabels.getJavaElementName(fMethodName)));
				...
				if (fImportRewriter.hasRecordedChanges()) {
					...
					result.addTextEditGroup(
							new TextEditGroup(RefactoringCoreMessages.ExtractMethodRefactoring_organize_imports,
									new TextEdit[] { edit }));
				}
	...
	public static ASTNode[] getSelectNodes(MethodDeclaration cUnit) {
		List<ASTNode> astNodes = cUnit.getBody().statements();
		List<ASTNode> tempAstNodes = new ArrayList<>();
		for (int j = 0; j < astNodes.size(); j++) {
			System.out.println("fSelectionStart:" + fSelectionStart);
			System.out.println("fSelectionLength:" + fSelectionLength);
			System.out.println("tempStart:" + astNodes.get(j).getStartPosition());
			System.out.println("tempLength:" + astNodes.get(j).getStartPosition() + astNodes.get(j).getLength());
			int length = astNodes.get(j).getStartPosition() + astNodes.get(j).getLength();
			if (astNodes.get(j).getStartPosition() >= fSelectionStart && length <= fSelectionLength) {
				tempAstNodes.add(astNodes.get(j));
			}
		}

		ASTNode[] nodes = new ASTNode[tempAstNodes.size()];
		for (int i = 0; i < tempAstNodes.size(); i++) {
			nodes[i] = tempAstNodes.get(i);
		}
		return nodes;
	}

	private static List<ASTNode> extractSelectedNodes(CompilationUnit compilationUnit, IDocument document,
			int selectionStart, int selectionLength) {
		List<ASTNode> selectedNodes = new ArrayList<>();
		...
		compilationUnit.accept(new ASTVisitor() {
			public boolean visitNode(ASTNode node) {
				...
				int nodeSelectionStart = Math.max(nodeStart, selectionStart);
				int nodeSelectionEnd = Math.min(nodeEnd, selectionEnd);

				if (nodeSelectionEnd > nodeSelectionStart) {
					selectedNodes.add(node);
					// Continue traversal to find other nodes
				}
				return true;
			}
		});
	...
	private ExtractMethodDescriptor getRefactoringDescriptor() {
		...
		final int flags = RefactoringDescriptor.STRUCTURAL_CHANGE | JavaRefactoringDescriptor.JAR_REFACTORING
		...
		final String label = method != null
				...
				: '{' + JavaElementLabelsCore.ELLIPSIS_STRING + '}';
		final String header = Messages.format(RefactoringCoreMessages.ExtractMethodRefactoring_descriptor_description,
				new String[] { BasicElementLabels.getJavaElementName(getSignature()), label,
						BindingLabelProviderCore.getBindingLabel(type, JavaElementLabelsCore.ALL_FULLY_QUALIFIED) });
		final JDTRefactoringDescriptorComment comment = new JDTRefactoringDescriptorComment(project, this, header);
		comment.addSetting(Messages.format(RefactoringCoreMessages.ExtractMethodRefactoring_name_pattern,
				BasicElementLabels.getJavaElementName(fMethodName)));
		comment.addSetting(Messages.format(RefactoringCoreMessages.ExtractMethodRefactoring_destination_pattern,
				BindingLabelProviderCore.getBindingLabel(type, JavaElementLabelsCore.ALL_FULLY_QUALIFIED)));
		String visibility = JdtFlags.getVisibilityString(fVisibility);
		if ("".equals(visibility)) //$NON-NLS-1$
			visibility = RefactoringCoreMessages.ExtractMethodRefactoring_default_visibility;
		comment.addSetting(
				Messages.format(RefactoringCoreMessages.ExtractMethodRefactoring_visibility_pattern, visibility));
		...
		final ExtractMethodDescriptor descriptor = RefactoringSignatureDescriptorFactory
				.createExtractMethodDescriptor(project, description, comment.asString(), arguments, flags);
	...
	private void initializeParameterInfos() {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			boolean isVarargs = declaration instanceof SingleVariableDeclaration
					...
					: false;
			ParameterInfo info = new ParameterInfo(argument, getType(declaration, isVarargs, false), argument.getName(),
					i);
			if (isVarargs) {
				vararg = info;
			} else {
...
//					ExtractMethodAnalyzer analyzer = new ExtractMethodAnalyzer(fCUnit,
//							Selection.createFromStartLength(duplicateStart, duplicateLength));
...
//								if (duplicateReturnTypeBinding
//										.equals(startNode.getAST().resolveWellKnownType("void"))) { //$NON-NLS-1$
...
//									BodyDeclaration originalEnclosingBodyDeclaration = fAnalyzer
//											.getEnclosingBodyDeclaration();
//									BodyDeclaration duplicateEnclosingBodyDeclaration = analyzer
//											.getEnclosingBodyDeclaration();
//									VariableDeclaration originalReturnNode = ASTNodes.findVariableDeclaration(
...
//											duplicateReturnValBinding, duplicateEnclosingBodyDeclaration);
//
//									if (originalReturnNode != null && duplicateReturnNode != null) {
//										boolean matches;
//										if (!fAnalyzer.getSelection().covers(originalReturnNode)
//												&& !analyzer.getSelection().covers(duplicateReturnNode)) {
//											// returned variables are defined outside of the selection => always OK
//											matches = true;
//										} else {
//											matches = matchesLocationInEnclosingBodyDecl(
//													originalEnclosingBodyDeclaration, duplicateEnclosingBodyDeclaration,
//													originalReturnNode, duplicateReturnNode);
//										}
//
//										if (matches) {
//											validDuplicates.add(duplicate);
...
//		}
	...
	private boolean matchesLocationInEnclosingBodyDecl(BodyDeclaration originalEnclosingBodyDeclaration,
			BodyDeclaration duplicateEnclosingBodyDeclaration, VariableDeclaration originalReturnNode,
			VariableDeclaration duplicateReturnNode) {
		boolean matches = true;
		ASTNode original = originalReturnNode;
		ASTNode dupliacte = duplicateReturnNode;

		// walk up the parent chains to check if the location of the return nodes in
		// their respective parent chains is same
		do {
			...
			if (originalParent != null && duplicateParent != null
				...
				if (originalLoc.isChildListProperty() && duplicateLoc.isChildListProperty()) {
					int indexOfOriginal = ((List<?>) originalParent.getStructuralProperty(originalLoc))
							.indexOf(original);
					int indexOfDuplicate = ((List<?>) duplicateParent.getStructuralProperty(duplicateLoc))
							.indexOf(dupliacte);
			...
			if ((originalEnclosingBodyDeclaration.equals(original)
					&& !duplicateEnclosingBodyDeclaration.equals(dupliacte))
					|| (!originalEnclosingBodyDeclaration.equals(original)
							&& duplicateEnclosingBodyDeclaration.equals(dupliacte))) {
				matches = false;
				break;
			}
		} while (!originalEnclosingBodyDeclaration.equals(original)
				&& !duplicateEnclosingBodyDeclaration.equals(dupliacte));
	...
	private void initializeDestinations() {
		...
		if (current != null && (decl instanceof MethodDeclaration || decl instanceof Initializer
	...
	private String getType(VariableDeclaration declaration, boolean isVarargs) {
		String type = ASTNodes.asString(ASTNodeFactory.newType(declaration.getAST(), declaration, fImportRewriter,
	...
	private String getType(VariableDeclaration declaration, boolean isVarargs, boolean isVarTypeAllowed) {
		if (isVarTypeAllowed) {
			...
			String type = ASTNodes.asString(ASTNodeFactory.newNonVarType(declaration.getAST(), declaration,
					fImportRewriter, new ContextSensitiveImportRewriteContext(declaration, fImportRewriter)));
			if (isVarargs)
				return type + ParameterInfo.ELLIPSIS;
			else
				return type;
	...
	private MethodDeclaration createNewMethod(ASTNode[] selectedNodes, String lineDelimiter, TextEditGroup substitute)
		...
		if (fGenerateJavadoc) {
			AbstractTypeDeclaration enclosingType = ASTNodes.getParent(fAnalyzer.getEnclosingBodyDeclaration(),
					AbstractTypeDeclaration.class);
			String string = CodeGeneration.getMethodComment(fCUnit, enclosingType.getName().getIdentifier(), result,
					null, lineDelimiter);
			if (string != null) {
	...
	private MethodDeclaration createNewMethodDeclaration() {
		...
		ImportRewriteContext context = new ContextSensitiveImportRewriteContext(enclosingBodyDeclaration,
				fImportRewriter);
	...
	private Block createMethodBody(ASTNode[] selectedNodes, TextEditGroup substitute, int modifiers) {
		...
		if (extractList.size() <= 0) {
			System.out.println("extract is null");
		}

		for (int a = 0; a < extractList.size(); a++) {
			System.out.println("extract:" + extractList.get(a));
			Statement statement = extractList.get(a);
			fRewriter.remove(statement, null);// remove statement
			result.statements().add((Statement) fRewriter.createCopyTarget(statement));
		}
		...
		if (callNodes.length == 1) {
			...
			System.out.println("replacementNode:" + callNodes);
		} else {
			replacementNode = fRewriter.createGroupNode(callNodes);
			System.out.println("replacementNode2:" + replacementNode);
		}
		if (extractsExpression) {
			...
			if (binding != null && (!binding.isPrimitive() || !"void".equals(binding.getName()))) { //$NON-NLS-1$
				ReturnStatement rs = fAST.newReturnStatement();
				rs.setExpression(
						(Expression) fRewriter.createMoveTarget(ASTNodes.getUnparenthesedExpression(selectedNodes[0])));
				statements.insertLast(rs, null);
			} else {
				ExpressionStatement st = fAST
						.newExpressionStatement((Expression) fRewriter.createMoveTarget(selectedNodes[0]));
			...
			if (fAnalyzer != null && fAnalyzer.getReturnTypeBinding() != null && fAST != null) {
				...
				if (selectedNodes.length == 1) {
					...
					if (selectedNodes[0].getLocationInParent() == LambdaExpression.BODY_PROPERTY) {
						if (replacementNode instanceof ExpressionStatement) {
							fRewriter.replace(selectedNodes[0], ((ExpressionStatement) replacementNode).getExpression(),
									substitute);
						} else if (replacementNode instanceof ReturnStatement) {
							fRewriter.replace(selectedNodes[0], ((ReturnStatement) replacementNode).getExpression(),
									substitute);
						}
					} else {
					...
					ASTNode toMove = source.createMoveTarget(selectedNodes[0], selectedNodes[index], replacementNode,
							substitute);
	...
	private VariableDeclarationStatement createDeclaration(IVariableBinding binding, Expression intilizer) {
		VariableDeclaration original = ASTNodes.findVariableDeclaration(binding,
				fAnalyzer.getEnclosingBodyDeclaration());
	...
	private RefactoringStatus initialize(JavaRefactoringArguments arguments) {
		...
		if (destination != null && destination.length() == 0) {
			...
			try {
				...
				return RefactoringStatus.createFatalErrorStatus(Messages.format(
						RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_DESTINATION));
			}
			fDestinationIndex = index;
		}
		...
		if (replace == null)
			return RefactoringStatus.createFatalErrorStatus(Messages
					.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_REPLACE));
		...
		if (comments == null)
			return RefactoringStatus.createFatalErrorStatus(Messages
					.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_COMMENTS));
		...
		if (exceptions == null)
			return RefactoringStatus.createFatalErrorStatus(Messages
					.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_EXCEPTIONS));
