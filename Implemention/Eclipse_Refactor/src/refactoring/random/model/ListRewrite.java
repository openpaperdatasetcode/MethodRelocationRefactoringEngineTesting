...
public class ListRewrite {
	 ...
	 * 
	 * @return the node that contains the list for which this list rewriter was
	 *         created
	...
	/**
	 * Returns the property of the parent node for which this list rewriter was
	 * created.
	 *
	 * @return the property of the parent node for which this list rewriter was
	 *         created
	 * @see #getParent()
	 * @since 3.1
	 */
	 ...
	 *                  node in this list or (since 3.4) a new node already inserted
	 ...
	 *                                  described modification is invalid (not a
	 *                                  member of this node's original list)
	 */
	 ...
	 *
	 * @param node        the node being removed. The node can either be an original
	 *                    node in this list or (since 3.4) a new node already
	 ...
	 *                                  modification is otherwise invalid (not a
	 *                                  member of this node's original list)
	 */
	 ...
	 * node <code>previousElement</code> must be in the list, either as an original
	 * or as a new node that has been inserted. The inserted node must either be
	 * brand new (not part of the original AST) or a placeholder node (for example,
	 * one created by {@link ASTRewrite#createCopyTarget(ASTNode)},
	 * {@link ASTRewrite#createMoveTarget(ASTNode)}, or
	 * {@link ASTRewrite#createStringPlaceholder(String, int)}). The AST itself is
	 * not actually modified in any way; rather, the rewriter just records a note
	 ...
	 * @param previousElement the element after which the given node is to be
	 *                        inserted
	 * @param editGroup       the edit group in which to collect the corresponding
	 *                        text edits, or <code>null</code> if ungrouped
	 * @throws IllegalArgumentException if the node or element is null, or if the
	 *                                  node is not part of this rewriter's AST, or
	 *                                  if the inserted node is not a new node (or
	 *                                  placeholder), or if
	 *                                  <code>previousElement</code> is not a member
	 *                                  of the list (original or new), or if the
	 ...
	 * node <code>nextElement</code> must be in the list, either as an original or
	 * as a new node that has been inserted. The inserted node must either be brand
	 * new (not part of the original AST) or a placeholder node (for example, one
	 * created by {@link ASTRewrite#createCopyTarget(ASTNode)},
	 * {@link ASTRewrite#createMoveTarget(ASTNode)}, or
	 * {@link ASTRewrite#createStringPlaceholder(String, int)}). The AST itself is
	 ...
	 * @param editGroup   the edit group in which to collect the corresponding text
	 *                    edits, or <code>null</code> if ungrouped
	 * @throws IllegalArgumentException if the node or next element is null, or if
	 *                                  the node is not part of this rewriter's AST,
	 *                                  or if the inserted node is not a new node
	 *                                  (or placeholder), or if
	 *                                  <code>nextElement</code> is not a member of
	 *                                  the list (original or new), or if the
	 *                                  described modification is otherwise invalid
	 */
	 ...
	 * @throws IllegalArgumentException if the node is null, or if the node is not
	 *                                  part of this rewriter's AST, or if the
	 *                                  inserted node is not a new node (or
	 *                                  placeholder), or if the described
	 *                                  modification is otherwise invalid (not a
	 *                                  member of this node's original list)
	 ...
	 *                  inserted nodes; <code>-1</code> indicates insertion as the
	 *                  last element
	 * @param editGroup the edit group in which to collect the corresponding text
	 *                  edits, or <code>null</code> if ungrouped
	 * @throws IllegalArgumentException  if the node is null, or if the node is not
	 ...
	 *                                   modification is otherwise invalid (not a
	 *                                   member of this node's original list)
	 * @throws IndexOutOfBoundsException if the index is negative and not -1, or if
	 *                                   it is larger than the size of the combined
	 *                                   list
	 */
	...
	private void validatePropertyType(ASTNode node) {
		...
		if (!this.childListProperty.getElementType().isAssignableFrom(node.getClass())) {
			String message = node.getClass().getName() + " is not a valid type for " //$NON-NLS-1$
					+ this.childListProperty.getNodeClass().getName() + " property '" + this.childListProperty.getId() + "'. Must be " //$NON-NLS-1$ //$NON-NLS-2$
	...
	private ASTNode createTargetNode(ASTNode first, ASTNode last, boolean isMove, ASTNode replacingNode,
		...
		CopySourceInfo info = getRewriteStore().createRangeCopy(this.parent, this.childListProperty, first, last,
				isMove, internalPlaceHolder, replacingNode, editGroup);
	 ...
	 *                                  node are <code>null</code>, if a node is not
	 *                                  a child of the current list or if the first
	 *                                  node is not before the last node. An
	 *                                  <code>IllegalArgumentException</code> is
	 *                                  also thrown if the copied range is
	 *                                  overlapping with an other moved or copied
	 *                                  range.
	 */
	 ...
	 *                                  node is not before the last node. An
	 *                                  <code>IllegalArgumentException</code> is
	 *                                  also thrown if the moved range is
	 *                                  overlapping with an other moved or copied
	 *                                  range.
	 *
	 * @since 3.1
	 ...
	 * @param first         the node that starts the range
	 * @param last          the node that ends the range
	 * @param replacingNode a node that is set at the location of the moved nodes or
	 *                      <code>null</code> to remove all nodes
	 * @param editGroup     the edit group in which to collect the corresponding
	 *                      text edits fro a replace, or <code>null</code> if
	 *                      ungrouped
	 * @return the new placeholder node
	 * @throws IllegalArgumentException An exception is thrown if the first or last
	 *                                  node are <code>null</code>, if a node is not
	 ...
	 * </code>                       is also thrown if the moved range is
	 *                                  overlapping with an other moved or copied
	 *                                  range.
	 *
	 * @since 3.1
	...
	/*
	 * Heuristic to decide if a inserted node is bound to previous or the next
	 * sibling.
	 */
