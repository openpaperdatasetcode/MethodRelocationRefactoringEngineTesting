...
//			JavaManipulationPlugin.log(exception);
	...
	// the checks depend on invocation context and therefore can't be done in
	// SourceAnalyzer
	private void checkMethodDeclaration(RefactoringStatus result, int severity) {
		...
		// it is not allowed to inline constructor invocation only if it is used for
		// class instance creation
		// if constructor is invoked from another constructor then we can inline such
		// invocation
		if (fInvocation.getNodeType() != ASTNode.CONSTRUCTOR_INVOCATION && methodDeclaration.isConstructor()) {
			result.addEntry(new RefactoringStatusEntry(severity, RefactoringCoreMessages.CallInliner_constructors,
		...
		if (fSourceProvider.hasSuperMethodInvocation() && fInvocation.getNodeType() == ASTNode.METHOD_INVOCATION) {
			...
			if (receiver instanceof ThisExpression) {
				result.addEntry(new RefactoringStatusEntry(severity,
			...
			if (isAssignment(parent) || isSingleDeclaration(parent)) {
				...
				if (!fSourceProvider.isSimpleFunction()) {
					if (isMultiDeclarationFragment(parent)) {
						...
						addEntry(result, RefactoringCoreMessages.CallInliner_field_initializer_simple,
				...
				if (isFieldDeclaration) {
					...
					for (int i = 0; i < argumentsCount; i++) {
						...
						if (parameter.isWrite()) {
							addEntry(result, RefactoringCoreMessages.CallInliner_field_initialize_write_parameter,
									RefactoringStatusCodes.INLINE_METHOD_FIELD_INITIALIZER, severity);
							return;
						}
					}
					if (fLocals.size() > 0) {
						addEntry(result, RefactoringCoreMessages.CallInliner_field_initialize_new_local,
								RefactoringStatusCodes.INLINE_METHOD_FIELD_INITIALIZER, severity);
						return;
					}
					// verify that the field is not referenced by the initializer method
					VariableDeclarationFragment variable = (VariableDeclarationFragment) ASTNodes.getParent(fInvocation,
							ASTNode.VARIABLE_DECLARATION_FRAGMENT);
					if (fSourceProvider.isVariableReferenced(variable.resolveBinding())) {
						addEntry(result, RefactoringCoreMessages.CallInliner_field_initialize_self_reference,
								RefactoringStatusCodes.INLINE_METHOD_FIELD_INITIALIZER, severity);
						return;
					}
				}
	...
	private void addEntry(RefactoringStatus result, String message, int code, int severity) {
		result.addEntry(new RefactoringStatusEntry(severity, message, JavaStatusContext.create(fCUnit, fInvocation),
				CorextCore.getPluginId(), code, null));
	}
...
//			Assert.isTrue(false, "Should not happen"); //$NON-NLS-1$
	...
	public RefactoringStatus perform(TextEditGroup textEditGroup) throws CoreException {
		RefactoringStatus result = new RefactoringStatus();
//		System.out.println("fInvocation:" + fInvocation);
//		System.out.println("fInvocationScope:" + fInvocationScope);
		fTargetNode();
		fContext = new CallContext(fInvocation, fInvocationScope, fTargetNode.getNodeType(), fImportRewrite);
//		System.out.println("fContext:" + fContext);
	...
	private void computeRealArguments() {
		...
		for (int i = 0; i < (needsVarargBoxing ? varargIndex : arguments.size()); i++) {
			...
			if (canInline(expression, parameter) && !canNotInline.contains(expression)) {
				...
				VariableDeclarationStatement local = createLocalDeclaration(parameter.getTypeBinding(), name,
...
//			decl.setType(type);
	...
	private boolean needsVarargBoxing(List<Expression> arguments) {
		if (!fSourceProvider.isVarargs())
			return false;
		/*
		 * if (!fSourceProvider.hasArrayAccess()) return false;
		 */
...
//			return !fTypeEnvironment.create(argument).canAssignTo(fTypeEnvironment.create(parameter));
	...
	private void computeReceiver() throws BadLocationException, org.eclipse.jface.text.BadLocationException {
		...
		case 0:
			...
			// local.
			fLocals.add(createLocalDeclaration(receiver.resolveTypeBinding(), fInvocationScope.createName("r", true), //$NON-NLS-1$
		...
		default:
			...
			fLocals.add(createLocalDeclaration(receiver.resolveTypeBinding(), local,
					(Expression) fRewrite.createCopyTarget(receiver)));
			...
			if (fContext.callMode == ASTNode.EXPRESSION_STATEMENT && fSourceProvider.hasReturnValue()) {
				if (fSourceProvider.mustEvaluateReturnedExpression()) {
					if (fSourceProvider.returnValueNeedsLocalVariable()) {
						...
						node = createLocalDeclaration(invocation.getReturnType(),
				...
				if (needsExplicitCast(status)) {
					...
					if (NecessaryParenthesesChecker.needsParentheses(fSourceProvider.getReturnExpressions().get(0),
							castExpression, CastExpression.EXPRESSION_PROPERTY)) {
					...
					if (NecessaryParenthesesChecker.needsParentheses(castExpression, fTargetNode.getParent(),
							fTargetNode.getLocationInParent())) {
						ParenthesizedExpression parenthesized = ast.newParenthesizedExpression();
						parenthesized.setExpression((Expression) node);
						node = parenthesized;
					}
				} else if (fSourceProvider.needsReturnedExpressionParenthesis(fTargetNode.getParent(),
						fTargetNode.getLocationInParent())) {
	 ...
	 * @return <code>true</code> if explicit cast is needed otherwise
	 *         <code>false</code>
	 */
...
//			return false;
	...
	private VariableDeclarationStatement createLocalDeclaration(ITypeBinding type, String name,
		...
		VariableDeclarationStatement decl = (VariableDeclarationStatement) ASTNodeFactory
				.newStatement(fInvocation.getAST(), typeName + " " + name + ";"); //$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 * calling method since the assignments might be reorder. An example is: <code>
	 *   add((field=args).length,field.hashCode());
	 * </code> Field might not be initialized when the arguments are reorder in the
	 * called method.
	 * 
	...
	private Set<Expression> crossCheckArguments(List<Expression> arguments) {
		...
		// if there is only 1 argument that has assignment and no others use it, there
		// is no issue
		if (result.size() == 1) {
			return new HashSet<>();
		}
	...
	private void initializeInsertionPoint(int nos) {
		...
		// if we have a constructor invocation the invocation itself is already a
		// statement
		ASTNode parentStatement = fInvocation instanceof Statement ? fInvocation
...
//				Assert.isNotNull(currentStatement);
	...
	private boolean needsBlockAroundDanglingIf() {
		 ...
		 *
		 * Situation: boolean a, b; void toInline() { if (a) hashCode(); } void m() { if
		 * (b) toInline(); else toString(); } => needs block around inlined "if (a)..."
		 * to avoid attaching else to wrong if.
		 */
