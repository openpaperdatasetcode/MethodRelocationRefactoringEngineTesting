...
public class EclipseExtractTempRefactoring extends Refactoring {
	 ...
	 */
	public EclipseExtractTempRefactoring(ICompilationUnit unit, int selectionStart, int selectionLength) {
	...
	public EclipseExtractTempRefactoring(CompilationUnit astRoot, int selectionStart, int selectionLength) {
		Assert.isTrue(selectionStart >= 0);
		Assert.isTrue(selectionLength >= 0);
	...
	public EclipseExtractTempRefactoring(JavaRefactoringArguments arguments, RefactoringStatus status) {
	...
	public RefactoringStatus checkFinalConditions(IProgressMonitor pm) throws CoreException {
		try {
			...
			doCreateChange(new SubProgressMonitor(pm, 1));
			...
			return new RefactoringStatus();// result
		} finally {
			pm.done();
		}
	}
...
//			return result;
	...
	private void createAndInsertTempDeclaration() throws CoreException {
		...
		if (node instanceof SwitchStatement) {
			...
			for (IASTFragment iFragment : retainOnlyReplacableMatches(getMatchingFragments())) {
	...
	private VariableDeclarationStatement createTempDeclaration(Expression initializer) throws CoreException {
		...
		System.out.println("fTempName:" + fTempName);
		if (fTempName == null) {
//			fTempName =  GptApi.printMessage(exp.toString());
			System.out.println("fTempName is null");
		}
		fTempName = fTempName.replaceAll("\"", "");
	...
	private ASTNode getEnclosingBodyNode() throws JavaModelException {
		if (getSelectedExpression().getAssociatedNode() != null) {
	...
	private IASTFragment[] getMatchingFragments() throws JavaModelException {
		if (fReplaceAllOccurrences && getEnclosingBodyNode() != null) {
